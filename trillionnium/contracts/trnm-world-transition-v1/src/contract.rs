...
impl CanonicalPayloadV1 {
    pub fn new(
        ...
        validate_canonical_json("payload.canonical_json", &canonical_json)?;
    ...
    pub fn validate_with_limit(
        ...
        validate_canonical_json(field, &self.canonical_json)?;
...
impl WorldTransitionRequestV1 {
    ...
    pub fn to_canonical_json(&self) -> Result<String, ContractValidationError> {
        ...
        write!(output, ",\"expected_tick\":{}", self.expected_tick)
    ...
    pub fn new(code: TransitionErrorCodeV1, detail: impl AsRef<str>) -> Self {
        let code = if matches!(
            ...
            TransitionErrorCodeV1::InvalidCommand
                ...
                | TransitionErrorCodeV1::InternalUnavailable
        ) {
            code
        } else {
            TransitionErrorCodeV1::DomainRejected
...
impl WorldTransitionAcceptedV1 {
    ...
    pub fn validate(&self) -> Result<(), ContractValidationError> {
        ...
        match (&self.outcome_material, &self.world_outcome_hash) {
            (Some(outcome), Some(hash)) => {
                ...
                let expected =
                    outcome_hash(&self.ruleset_revision, &self.content_revision, outcome);
                if expected.as_str() != hash.as_str() {
    ...
    pub fn canonical_facts_json(&self) -> Result<String, ContractValidationError> {
        ...
        write!(output, ",\"next_tick\":{}", self.next_tick)
            .expect("writing to String cannot fail");
...
fn validate_canonical_json(
    ...
    if !canonical_json::validate(canonical_json) {
...
mod tests {
    ...
    #[test]
    fn noncanonical_payloads_fail_closed() {
        for payload in [
            "{ \"order\":1}",
            "{\"order\":}",
            "{\"b\":1,\"a\":2}",
            ...
            let error = CanonicalPayloadV1::new("trnm.rts.order.v1", payload)
    ...
    fn rejection_detail_is_bounded_and_control_free() {
        ...
        assert!(rejection.detail.chars().count() <= 256);
