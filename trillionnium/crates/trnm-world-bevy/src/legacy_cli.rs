pub fn run() {
    ...
    if matches!(
        ...
        Some(
            "classic-rts-production-asset-atlas"
                | "--classic-rts-production-asset-atlas"
                | "production-asset-atlas"
        )
    ) {
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-production-asset-atlas.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_production_asset_atlas_evidence_json(preview_path)
        );
        return;
    }
    if matches!(
    ...
    if matches!(
        ...
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-production-interaction-polish.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_production_interaction_polish_evidence_json(
                preview_path
            )
        );
        return;
    ...
    if matches!(
        ...
        Some(
            "classic-rts-full-screen-ui-replication"
                | "--classic-rts-full-screen-ui-replication"
                | "full-screen-ui-replication"
        )
    ) {
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-full-screen-ui-replication.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_full_screen_ui_replication_evidence_json(
                preview_path
            )
        );
        return;
    ...
    if matches!(
        ...
        Some(
            "classic-rts-match-setup-ui-replication"
                | "--classic-rts-match-setup-ui-replication"
                | "match-setup-ui-replication"
        )
    ) {
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-match-setup-ui-replication.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_match_setup_ui_replication_evidence_json(
                preview_path
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-continuous-player-flow.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_continuous_player_flow_evidence_json(preview_path)
        );
        return;
    }
    if matches!(
        ...
        Some(
            "classic-rts-live-session-playthrough"
                | "--classic-rts-live-session-playthrough"
                | "live-session-playthrough"
        )
    ) {
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-live-session-playthrough.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_live_session_playthrough_evidence_json(
                preview_path
            )
        );
        return;
    ...
    if matches!(
        ...
        Some(
            "classic-rts-openra-screen-for-screen-ui-replication"
                | "--classic-rts-openra-screen-for-screen-ui-replication"
                | "openra-screen-for-screen-ui-replication"
        )
    ) {
        let preview_path = args.get(1).map(String::as_str).unwrap_or(
            "acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-screen-for-screen-ui-replication.ppm",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_screen_for_screen_ui_replication_evidence_json(
                preview_path
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-replay-compat-adapter",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_replay_compat_adapter_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-imported-replay-reducer",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_imported_replay_reducer_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-imported-replay-repro-manifest",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_imported_replay_repro_manifest_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-imported-replay-artifact-bundle",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_imported_replay_artifact_bundle_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        args.first().map(String::as_str),
        Some(
            "classic-rts-openra-imported-replay-review-capsule"
                | "--classic-rts-openra-imported-replay-review-capsule"
        )
    ) {
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-imported-replay-review-capsule",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_imported_replay_review_capsule_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        args.first().map(String::as_str),
        Some(
            "classic-rts-openra-imported-replay-review-digest"
                | "--classic-rts-openra-imported-replay-review-digest"
        )
    ) {
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-imported-replay-review-digest",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_imported_replay_review_digest_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-order-replay-reducer",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_order_replay_reducer_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-openra-headless-comparison-harness",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_openra_headless_comparison_harness_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        args.first().map(String::as_str),
        Some(
            "classic-rts-bot-planner-action-executor" | "--classic-rts-bot-planner-action-executor"
        )
    ) {
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-bot-planner-action-executor",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_bot_planner_action_executor_evidence_json(
                preview_dir
            )
        );
        return;
    ...
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-bot-planner-executor-replay-determinism",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_bot_planner_executor_replay_determinism_evidence_json(preview_dir)
        );
        return;
    }
    if matches!(
        ...
        let preview_dir = args.get(1).map(String::as_str).unwrap_or(
            "../acceptance/S5_native_bevy_device/latest/bevy-classic-rts-multi-match-bot-executor-evaluation",
        );
        println!(
            "{}",
            trnm_world_bevy::native_classic_rts_multi_match_bot_executor_evaluation_evidence_json(
                preview_dir
            )
        );
        return;
