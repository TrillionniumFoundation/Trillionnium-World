    ...
    pub fn submit(
        ...
        let response = self
            ...
            .post(format!(
                ...
                self.base_url, self.match_id
            ))
            .header("x-trnm-player-session", &self.player_session)
            .json(&request)
            .send()
            .map_err(|error| format!("online command transport: {error}"))?;
...
fn fetch_snapshot(
    ...
    let response = client
        ...
        .send()
        .map_err(|error| format!("online snapshot transport: {error}"))?;
    let status = response.status();
    let body = response
        .text()
