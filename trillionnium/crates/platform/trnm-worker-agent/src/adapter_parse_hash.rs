//! Adapter transaction hash extraction helpers.
    ...
    let cleaned = raw
        .trim_matches(|c: char| {
            is_receipt_quote_wrapper(c)
                ...
                || c.is_control()
                || super::adapter_parse_normalization::is_invisible_filler(c)
        })
        ...
        loop {
            ...
            candidate = candidate.trim_start_matches(|ch: char| {
                ch.is_ascii_whitespace()
                    || ch.is_control()
                    || super::adapter_parse_normalization::is_invisible_filler(ch)
