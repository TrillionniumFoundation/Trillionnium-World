...
#[test]
fn replay_step_fails_closed_when_remote_head_hash_is_wrapped_before_resume_replay() {
    let mut invalid_head = head(5, 5_000);
    invalid_head.hash = " hash-5 ".into();
    ...
    let err = replay_step(&mut source, &mut sink, &mut cursor, Some(4), 6_000)
        .expect_err("wrapped remote head hash must fail closed before resume replay");
    ...
    assert_eq!(
        ...
        "resume replay must keep the cursor pinned when the remote head hash is wrapped"
    );
    assert!(
        sink.persisted.is_empty(),
        "wrapped remote head metadata must not permit any replay persistence"
    );
}
