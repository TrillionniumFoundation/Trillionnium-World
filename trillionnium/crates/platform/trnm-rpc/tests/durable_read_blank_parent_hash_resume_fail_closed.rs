...
#[test]
fn replay_step_fails_closed_when_resume_block_parent_hash_is_blank() {
    let mut invalid_block = block(5, 5_000);
    invalid_block.parent_hash = Some(String::new());
    ...
    let err = replay_step(&mut source, &mut sink, &mut cursor, Some(4), 6_000)
        .expect_err("blank parent hash must fail closed");
    ...
    assert_eq!(
        ...
        "resume replay must keep the cursor pinned on the blank parent-hash height"
    );
    assert!(
        sink.persisted.is_empty(),
        "blank parent linkage must not persist a malformed replay block"
    );
}
