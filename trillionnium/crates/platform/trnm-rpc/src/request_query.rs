use crate::ingress::load_ingress_records;
use crate::node_events::load_node_events;
use crate::rpc_util::clamp_limit;
use crate::taskview::filtered_node_events_for_task;
use crate::{
    normalize_actor_or_signer, push_tail_limited, EventQueryResponse, MessageIngressRecord,
    MessageRequestQueryResponse, NodeEventScanMode, RequestFullQueryResponse,
    QUERY_FULL_LIMIT_DEFAULT, QUERY_FULL_LIMIT_MAX,
};
use anyhow::{bail, Result};

fn find_request_record(request_id: &str) -> Result<MessageIngressRecord> {
...
pub(crate) fn handle_query_request(request_id: &str) -> Result<()> {
    let rec = find_request_record(request_id)?;
    let out = request_query_response(&rec);
    println!("{}", serde_json::to_string_pretty(&out)?);
    Ok(())
}

pub(crate) fn handle_query_request_full(request_id: &str, limit: usize) -> Result<()> {
    ...
    let out = RequestFullQueryResponse {
        request: request_query_response(&rec),
