...
- Current native snapshot (2026-07-13): schema revision 12, Term Exchange protocol v2, RTS/checkpoint v16, Online Authority v2, Online Product v2, Online Operations v2 and Online Production v2. The closed-alpha path now includes signer-possession/CEX-registry proof, PostgreSQL-distributed admission, serialized fleet startup migrations, capacity sampling, durable moderation shifts and a native delayed-spectator view in addition to Production v1 signer rotation and authoritative gameplay. See `GAME_STATUS.md` for exact evidence and open boundaries; real-human completion, KMS/HSM custody, a second physical host, public WAF/DDoS/capacity, staffed safety and player listings remain release-gated.
