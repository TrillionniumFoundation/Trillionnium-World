...
- Current native snapshot (2026-07-13): schema revision 12, Term Exchange protocol v2, RTS/checkpoint v16, Online Authority v2, Online Product v2 and Online Operations v1. The closed-alpha path includes invite accounts, native text login with Linux kernel-keyring recovery, private `coop_vs_ai`, ranked authoritative PvP, active-season leaderboard, replay-bound reports, integrity/moderation audit and a same-host two-process fleet/failover gate. See `GAME_STATUS.md` for exact evidence and open boundaries; public beta, chat/guild, staffed safety, cross-host regional HA and player listings remain release-gated.
