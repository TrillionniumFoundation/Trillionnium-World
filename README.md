...
Classic RTS OpenRA port note: `scripts/check_trillionnium_world_bevy_classic_rts_openra_engine_port_asset_parity.sh` adds the first explicit scoped Rust OpenRA engine-port foundation gate. It binds ModData/ruleset/actor/world/order/chrome/widget/sprite/palette/asset-loader/replay surfaces, reads the canonical OpenRA screen-for-screen UI evidence, and proves pixel-perfect parity over 12 Trillionnium-owned OpenRA-compatible PPM asset samples with zero sha or pixel mismatches. Release-review packet integrity binds `openra_engine_port_asset_parity_semantics` and `openra_engine_port_asset_parity_ppm_semantics`; the gate claims only the Rust foundation port and owned-asset parity scope, while full C# OpenRA engine parity, OpenRA/Westwood asset copying, binary replay/network parity, Android S5, and public launch remain unclaimed.
