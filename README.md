...
- Current native snapshot (2026-07-13): schema revision 12, Term Exchange protocol v2, RTS/checkpoint v16, Online Authority v2, Online Product v2 and Online Operations v2. The closed-alpha path includes invite accounts, native text login with Linux kernel-keyring recovery, private `coop_vs_ai`, ranked authoritative PvP, auditable season rotation, integrity-verified replay frames/F9 inspection, enforcement appeals, anti-collusion controls and epoch-fenced same-host fleet leases. See `GAME_STATUS.md` for exact evidence and open boundaries; real-human completion, public beta, chat/guild, staffed safety, KMS/HSM, public DDoS/capacity, cross-host regional HA and player listings remain release-gated.
