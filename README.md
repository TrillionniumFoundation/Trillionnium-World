...
- Current native snapshot (2026-07-23): schema revision 12, Term Exchange protocol v2, RTS/checkpoint v16, Online Authority v3, Online Product v2, Online Operations v2 and Online Production v2. Release-bound 100-ms PostgreSQL RTT evidence is green, while both 24-hour endurance attempts remain invalid and the new real-clock frame/input/network timing v3 gate still needs promoted native evidence. See `GAME_STATUS.md` for exact evidence and open boundaries; real-human completion, distributable multi-OS packages, KMS/HSM custody, a second physical host, public WAF/DDoS/capacity, staffed safety and player listings remain release-gated.
