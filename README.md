...
- Current native snapshot (2026-07-13): schema revision 12, Term Exchange protocol v2, RTS/checkpoint v16, Online Authority v2, Online Product v2, Online Operations v2 and Online Production v1. The closed-alpha path now includes an isolated rotating entitlement signer, bounded production ingress, automatic season transition, targeted delayed spectating, SLA escalation and physical-host-aware fleet ownership in addition to the existing authoritative gameplay. See `GAME_STATUS.md` for exact evidence and open boundaries; real-human completion, KMS/HSM custody, a second physical host, public WAF/DDoS/capacity, staffed safety and player listings remain release-gated.
