...
- **Current game boundary gate**: `scripts/check_trnm_game_product.sh` verifies the eight-crate product and rejects removed World/RTS dependencies. `scripts/check_trillionnium_world_client_boundary.sh` is a historical pre-cleanup checker and grants no current product credit.
- **Current game depth evidence**: `GAME_STATUS.md` enumerates the revision-12 branch-consequence/multi-beat-chapter/epilogue/autonomous-social/visible-caravan state, typed economic outbox/receipt/reconciliation boundary and the shared worker/job-command/construction/structure, meaningful-seed, normal-growth terminal, 64-sample economy-matrix and checkpointed chunked-replay authority. The automated real-map/faction/spawn/seed matrix is regression evidence, not a public-launch or esports-balance claim.
...
- **Current interpretation**: the eight-crate native product is locally playable and includes a bounded Online Authority v1 vertical slice; `public_launch_ready=false` and `android_s5_real_device_claimed=false`. Historical release-review packets that credited the old CEX World adapter remain stale; current credit requires the game-owned protocols, current CEX build and real-ledger/server E2E.
- **What it proves**: current product tests, deterministic E2E, release build, live desktop window, eight-crate boundary and local CEX/ledger failure tests are local evidence only. CEX no longer imports the removed World traits; this does not prove public CEX deployment, production account issuance, legal/commercial readiness or public player trading.
