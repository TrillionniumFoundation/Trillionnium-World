...
CURRENT_INTEGRATION = {
    ...
    "pull_request": 8,
    "commit": "ae531f5022f6fb190105986959a5e5543ab47d35",
}
...
def exact_dependency(actual: dict[str, Any], expected: dict[str, Any], label: str) -> None:
    ...
    if commit is not None:
        require(isinstance(commit, str) and SHA1.fullmatch(commit) is not None, f"{label} commit is not a Git SHA")
    tree = actual.get("tree")
    if tree is not None:
        require(isinstance(tree, str) and SHA1.fullmatch(tree) is not None, f"{label} tree is not a Git SHA")
...
def validate(root: Path) -> None:
    ...
    require(isinstance(components, list) and {"rustfmt", "clippy"}.issubset(set(components)), "toolchain must include rustfmt and clippy")
    ...
    require(object_value(boundary.get("remote"), "boundary remote").get("canonical_slug") == REPOSITORY, "canonical repository drift")
    ...
    require(release_boundary.get("production_authorization") == "not_granted", "boundary production authorization drift")
    ...
    for relative, expected_class in (
        ...
        require(entry.get("status") in {"current", "current-candidate"}, f"{relative} catalog status drift")
    ...
    require(review.get("production_authorization") == "not_granted", "V6 review authorization drift")
    require(object_value(ledger.get("toolchain"), "V6 toolchain").get("current_candidate") == SAFE_TOOLCHAIN, "V6 toolchain drift")
    ...
    require(source.get("qualification_scope") == "source_and_documentation_only", "repository source scope overclaim")
    ...
    for key in ("exact_head_ci_closed", "prospective_merge_ci_closed", "independent_review_closed", "server_governance_closed"):
        require(evidence.get(key) is False, f"unproved repository denominator promoted: {key}")
    require(set(evidence.get("observed_main_required_contexts", [])) == OBSERVED_MAIN_CONTEXTS, "observed main contexts drift")
    require(evidence.get("desired_contract_matches_observation") is False, "server governance mismatch was hidden")
    ...
    exact_dependency(object_value(dependencies.get("integration"), "V6 Integration"), CURRENT_INTEGRATION, "V6 Integration")
    for name in ("cex", "nakama", "chain", "integration"):
        ...
        require(item.get("production_authorization") == "not_granted", f"{name} production authorization drift")
    closure = object_value(ledger.get("closure"), "V6 closure")
    require(closure.get("repository_source_documentation_closed_candidate") is True, "source/documentation candidate closure lost")
    for key in (
    ...
    require(closure.get("production_authorization") == "not_granted", "V6 production authorization drift")

    cex_lock = load(root, CEX_LOCK)
    require(cex_lock.get("schema") == "trnm_world_cex_pending_component_lock_v2", "current CEX lock schema drift")
    require(cex_lock.get("lock_state") == "coordination_only_blocked", "CEX lock must remain coordination-only")
    require(cex_lock.get("selection_state") == "observed_unqualified_candidate", "CEX selection state drift")
    world = object_value(cex_lock.get("world"), "CEX lock World")
    require(world.get("repository") == REPOSITORY and world.get("pull_request") == PULL_REQUEST and world.get("branch") == BRANCH, "CEX lock World identity drift")
    exact_dependency(object_value(cex_lock.get("cex"), "CEX lock CEX"), CURRENT_CEX, "CEX lock CEX")
    require(len(list_value(cex_lock.get("required_before_selection"), "CEX selection requirements")) >= 8, "CEX qualification requirements weakened")
    prohibited = set(list_value(cex_lock.get("prohibited_promotions"), "CEX prohibited promotions"))
    require({"qualified", "trusted_settlement", "production_ready", "custody_approved", "production_authorization"}.issubset(prohibited), "CEX prohibited promotions weakened")
    require(cex_lock.get("world_adoption") == "disabled", "World CEX adoption enabled without qualification")
    for key in ("qualified", "trusted_settlement", "custody_approved", "public_player_market", "production_ready"):
        require(cex_lock.get(key) is False, f"CEX lock overclaim: {key}")
    require(cex_lock.get("production_authorization") == "not_granted", "CEX lock production authorization drift")

    protection = load(root, PROTECTION)
    require(protection.get("server_state") == "not_enforced_until_api_readback", "desired branch protection misrepresented as enforced")
    require(protection.get("production_authorization") == "not_granted", "protection contract authorization drift")
    ...
    require("public online, trusted settlement, the public player market and commercial release remain no-go / disabled" in release.lower(), "release NO-GO statement missing")
    require("/Users/" not in release and ".openclaw/workspace" not in release, "release truth contains a personal path")
...
def main() -> int:
    ...
    try:
        validate(args.root)
    except (ActiveClosureFailure, OSError, UnicodeError, StrictJsonError, tomllib.TOMLDecodeError) as error:
        print(f"TRNM World active closure truth: FAIL: {error}", file=sys.stderr)
        return 1
    print("TRNM World active closure truth: PASS (toolchain, release truth, V6 ledger and current CEX coordination lock)")
    print("Hosted CI, server governance, independent review, cross-repository and external evidence remain separate.")
    return 0
