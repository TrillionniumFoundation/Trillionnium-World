#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SCRIPT="$ROOT/scripts/check_trillionnium_world_release_review_ci_gate.sh"

required_lines=(
  'trillionnium_world_release_review_ci_gate_v1'
  'release-review-ci-gate.json'
  'release_review_acceptance_lock.sh'
  'trnm_acquire_release_review_acceptance_lock "$ACCEPTANCE_DIR"'
  'release-review-ci-gate-workflow-script-refs.json'
  'elapsed_millis'
  'elapsed_seconds'
  'total_elapsed_millis'
  'check_count'
  'checks_total: $check_count'
  'failed_check_count'
  'checks_failed: $failed_check_count'
  '.failed_check_count // ((.failures // []) | length)'
  '--slurpfile checks "$CHECKS_FILE"'
  'release-review-ci-gate-world-bevy-release-build.log'
  'if [[ -z "${TRNM_WORLD_BEVY_ARTIFACT_BIN:-}" ]]'
  'cargo build --release -p trnm-world-bevy --bin trnm-world-bevy'
  ') >"$WORLD_BEVY_RELEASE_BUILD_LOG" 2>&1'
  'export TRNM_WORLD_BEVY_ARTIFACT_BIN="$ROOT/target/release/trnm-world-bevy"'
  'slow_check_threshold_millis'
  'slow_checks'
  'check_trillionnium_world_release_review_packet_integrity.sh'
  'check_trillionnium_world_next_execution_plan.sh'
  'world_next_execution_plan_script_contract_guard_test.sh'
  'world_next_execution_plan_contract_guard'
  'world_next_execution_plan'
  'release_review_quickcheck_gate'
  'release_review_status_gate'
  'release_review_convergence_gate'
  'bevy_classic_rts_first_contact_basin_spec_packet_refresh_gate'
  'bevy_classic_playtest_handoff_packet_packet_refresh_gate'
  'release_packet_refresh_gate'
  'TRNM_RELEASE_REVIEW_PACKET_REFRESH_INPUTS=0'
  'check_trillionnium_world_release_review_packet.sh'
  'packet_integrity_semantic_fixture_summary_refresh_gate'
  'TRNM_RELEASE_REVIEW_PACKET_INTEGRITY_SEMANTIC_FIXTURE_SUMMARIES_ONLY=1'
  'TRNM_RELEASE_REVIEW_PACKET_INTEGRITY_SEMANTIC_FIXTURE_PACKET_JSON="$ACCEPTANCE_DIR/release-review-packet.json"'
  'TRNM_RELEASE_REVIEW_PACKET_INTEGRITY_SEMANTIC_FIXTURE_PACKET_MD="$ACCEPTANCE_DIR/release-review-packet.md"'
  'packet_integrity_gate "$ROOT/scripts/check_trillionnium_world_release_review_packet_integrity.sh" --no-refresh'
  'check_trillionnium_world_release_review_checkpoint_manifest.sh'
  'check_trillionnium_world_client_boundary.sh'
  'client_boundary_script_contract_guard_test.sh'
  'client_boundary_contract_guard'
  'client_boundary_gate'
  'check_trillionnium_world_cex_adapter_readiness.sh'
  'cex_adapter_readiness_script_contract_guard_test.sh'
  'cex_adapter_readiness_gate'
  'release_review_status_script_contract_guard_test.sh'
  'release_review_convergence_script_contract_guard_test.sh'
  'release_review_packet_script_contract_guard_test.sh'
  'release_review_packet_integrity_script_contract_guard_test.sh'
  'check_trillionnium_world_release_review_packet_integrity_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_bot_executor_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_bot_executor_matrix_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_bot_gap_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_control_loop_semantic_fixture.sh'
	  'check_trillionnium_world_release_review_packet_integrity_selection_minimap_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_build_lifecycle_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_tech_tree_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_projectile_ability_semantic_fixture.sh'
  'check_trillionnium_world_release_review_packet_integrity_semantic_fixture_suite.sh'
  'release_review_packet_integrity_drift_guard_test.sh'
  'release_review_packet_integrity_semantic_guard_test.sh'
  'release_review_packet_integrity_bot_executor_semantic_guard_test.sh'
  'release_review_packet_integrity_bot_executor_matrix_semantic_guard_test.sh'
  'release_review_packet_integrity_bot_gap_semantic_guard_test.sh'
  'release_review_packet_integrity_control_loop_semantic_guard_test.sh'
	  'release_review_packet_integrity_selection_minimap_semantic_guard_test.sh'
  'release_review_packet_integrity_build_lifecycle_semantic_guard_test.sh'
  'release_review_packet_integrity_tech_tree_semantic_guard_test.sh'
  'release_review_packet_integrity_projectile_ability_semantic_guard_test.sh'
  'packet_integrity_semantic_fixture_suite_gate'
	  'release_review_checkpoint_manifest_script_contract_guard_test.sh'
  'checkpoint_manifest_contract_guard'
  'checkpoint_manifest'
  'public_launch_bundle_negative_fixtures_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_bundle_negative_fixtures.sh'
  'public_launch_evidence_bundle_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_evidence_bundle.sh'
  'public_launch_template_negative_fixtures_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_template_negative_fixtures.sh'
  'public_launch_evidence_kit_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_evidence_kit.sh'
  'public_launch_operator_handoff_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_operator_handoff.sh'
  'TRNM_PUBLIC_LAUNCH_OPERATOR_HANDOFF_REFRESH_INPUTS=0'
  'public_launch_operator_handoff_contract_guard'
  'public_launch_operator_handoff_gate'
  'public_launch_readiness_script_contract_guard_test.sh'
  'public_launch_readiness_contract_guard'
  'public_launch_blocker_consistency_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_blocker_consistency.sh'
  'public_launch_status_only_fixture_guard_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_status_only_fixtures.sh'
  'cohort_commercial_evidence_collection_script_contract_guard_test.sh'
  'check_trillionnium_world_cohort_commercial_evidence_collection.sh'
  'cohort_commercial_evidence_script_contract_guard_test.sh'
  'check_trillionnium_world_cohort_commercial_evidence.sh'
  'external_ops_evidence_collection_script_contract_guard_test.sh'
  'check_trillionnium_world_external_ops_evidence_collection.sh'
  'external_ops_evidence_script_contract_guard_test.sh'
  'check_trillionnium_world_external_ops_evidence.sh'
  's5_device_evidence_collector_script_contract_guard_test.sh'
  'check_trillionnium_world_s5_device_evidence.sh'
  's5_real_device_evidence_script_contract_guard_test.sh'
  'check_trillionnium_world_s5_real_device_evidence.sh'
  'halium_sidecar_runtime_dev_surface_script_contract_guard_test.sh'
  'halium_sidecar_runtime_dev_surface_operator_payload_fixture_test.sh'
  'halium_sidecar_runtime_dev_surface_unsafe_tar_fixture_test.sh'
  'check_trillionnium_world_halium_sidecar_runtime_dev_surface.sh'
  'halium_sidecar_runtime_dev_surface_contract_guard'
  'halium_sidecar_runtime_dev_surface_gate'
  'halium_sidecar_runtime_dev_surface_operator_payload_fixture'
  'halium_sidecar_runtime_dev_surface_unsafe_tar_fixture'
  'bevy_release_ci_artifact_wrapper_script_contract_guard_test.sh'
  'bevy_release_ci_artifact_wrapper_contract_guard'
  'keyboard_replay_script_contract_guard_test.sh'
  'bevy_keyboard_replay_contract_guard'
  'check_trillionnium_world_bevy_action_coach.sh'
  'player_hud_debug_layer_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_player_hud_debug_layer.sh'
  'bevy_player_hud_debug_layer_contract_guard'
  'bevy_player_hud_debug_layer_gate'
  'player_ui_rescue_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_player_ui_rescue.sh'
  'bevy_account_client_boundary_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_account_client_boundary.sh'
  'bevy_account_client_boundary_contract_guard'
  'bevy_account_client_boundary_gate'
  'bevy_account_title_flow_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_account_title_flow.sh'
  'bevy_account_title_flow_contract_guard'
  'bevy_account_title_flow_gate'
  'bevy_classic_foundation_artifact_wrapper_script_contract_guard_test.sh'
  'bevy_classic_foundation_artifact_wrapper_contract_guard'
  'check_trillionnium_world_bevy_classic_asset_pack.sh'
  'check_trillionnium_world_bevy_classic_manifest_lint.sh'
  'bevy_classic_animation_evidence_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_classic_animation_preview.sh'
  'check_trillionnium_world_bevy_classic_animation_selector.sh'
  'bevy_classic_player_motion_probe_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_classic_player_motion_probe.sh'
  'check_trillionnium_world_bevy_classic_input_frame_budget.sh'
  'check_trillionnium_world_bevy_classic_render_budget.sh'
  'bevy_classic_visual_foundation_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_classic_scene_preview.sh'
  'check_trillionnium_world_bevy_classic_model_catalog.sh'
  'check_trillionnium_world_bevy_classic_renderer_probe.sh'
  'check_trillionnium_world_bevy_classic_isometric_modeling.sh'
  'check_trillionnium_world_bevy_classic_rts_openra_like_core.sh'
  'check_trillionnium_world_bevy_classic_rts_control_loop.sh'
  'check_trillionnium_world_bevy_classic_rts_selection_minimap.sh'
  'check_trillionnium_world_bevy_classic_rts_build_lifecycle.sh'
  'check_trillionnium_world_bevy_classic_rts_tech_tree.sh'
  'check_trillionnium_world_bevy_classic_rts_projectile_ability.sh'
  'check_trillionnium_world_bevy_classic_rts_ai_skirmish_pressure.sh'
  'check_trillionnium_world_bevy_classic_rts_objective_victory_loop.sh'
  'check_trillionnium_world_bevy_classic_rts_creep_camp_terrain_route.sh'
  'check_trillionnium_world_bevy_classic_rts_fog_scouting_intel.sh'
  'check_trillionnium_world_bevy_classic_rts_enemy_base_tech_pressure.sh'
  'check_trillionnium_world_bevy_classic_rts_army_production_rally.sh'
  'check_trillionnium_world_bevy_classic_rts_base_assault_resolution.sh'
  'check_trillionnium_world_bevy_classic_rts_battle_aftermath.sh'
  'check_trillionnium_world_bevy_classic_rts_commander_progression.sh'
  'check_trillionnium_world_bevy_classic_rts_expansion_counterattack.sh'
  'check_trillionnium_world_bevy_classic_rts_tier_two_siege_push.sh'
  'check_trillionnium_world_bevy_classic_rts_siege_breach_counterplay.sh'
  'check_trillionnium_world_bevy_classic_rts_inner_lane_breakthrough.sh'
  'check_trillionnium_world_bevy_classic_rts_central_keep_pressure.sh'
  'check_trillionnium_world_bevy_classic_rts_central_keep_breakthrough.sh'
  'check_trillionnium_world_bevy_classic_rts_mirror_city_restoration.sh'
  'check_trillionnium_world_bevy_classic_rts_open_world_after_action.sh'
  'check_trillionnium_world_bevy_classic_rts_campaign_handoff.sh'
  'check_trillionnium_world_bevy_classic_rts_campaign_entry.sh'
  'check_trillionnium_world_bevy_classic_rts_visual_fidelity.sh'
  'check_trillionnium_world_bevy_classic_rts_production_art_replication.sh'
  'check_trillionnium_world_bevy_classic_rts_production_asset_atlas.sh'
  'check_trillionnium_world_bevy_classic_rts_production_ui_skin.sh'
  'check_trillionnium_world_bevy_classic_rts_production_interaction_polish.sh'
  'check_trillionnium_world_bevy_classic_rts_full_screen_ui_replication.sh'
  'check_trillionnium_world_bevy_classic_rts_shell_meta_ui_replication.sh'
  'check_trillionnium_world_bevy_classic_rts_match_setup_ui_replication.sh'
  'check_trillionnium_world_bevy_classic_rts_campaign_outcome_ui_readiness.sh'
  'check_trillionnium_world_bevy_classic_rts_campaign_ui_continuity.sh'
  'check_trillionnium_world_bevy_classic_rts_in_match_hud_state_replication.sh'
  'check_trillionnium_world_bevy_classic_rts_session_state_continuity.sh'
  'check_trillionnium_world_bevy_classic_rts_continuous_player_flow.sh'
  'check_trillionnium_world_bevy_classic_rts_live_session_playthrough.sh'
  'check_trillionnium_world_bevy_classic_rts_command_affordance.sh'
  'check_trillionnium_world_bevy_classic_rts_command_surface.sh'
  'check_trillionnium_world_bevy_classic_rts_structure_modeling.sh'
  'check_trillionnium_world_bevy_classic_rts_environment_life.sh'
  'check_trillionnium_world_bevy_classic_rts_worker_harvest_animation.sh'
  'check_trillionnium_world_bevy_classic_rts_ability_tooltip_telegraph.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_hotkey_feedback.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_recall_formation_preview.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_recall_override_preview.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_command_feedback_strip.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_command_feedback_lifecycle.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_command_history.sh'
  'check_trillionnium_world_bevy_classic_rts_control_group_command_history_prune.sh'
  'check_trillionnium_world_bevy_classic_rts_scrollable_map.sh'
  'check_trillionnium_world_bevy_classic_rts_camera_minimap_sync.sh'
  'check_trillionnium_world_bevy_classic_rts_action_cadence.sh'
  'check_trillionnium_world_bevy_classic_rts_unit_model_depth.sh'
  'check_trillionnium_world_bevy_classic_rts_action_sequence.sh'
  'check_trillionnium_world_bevy_classic_rts_npc_behavior.sh'
  'check_trillionnium_world_bevy_classic_rts_combat_impact.sh'
  'check_trillionnium_world_bevy_classic_rts_locomotion_blend.sh'
  'check_trillionnium_world_bevy_classic_rts_npc_transition.sh'
  'check_trillionnium_world_bevy_classic_rts_depth_readability.sh'
  'check_trillionnium_world_bevy_classic_rts_combat_readability_pressure_readiness.sh'
  'check_trillionnium_world_bevy_classic_rts_openra_parity_bridge.sh'
  'check_trillionnium_world_bevy_classic_rts_owned_replay_file.sh'
  'check_trillionnium_world_bevy_classic_rts_headless_replay_playback.sh'
  'check_trillionnium_world_bevy_classic_rts_natural_terminal_contract.sh'
  'check_trillionnium_world_bevy_classic_rts_native_bot_ai_planner.sh'
  'check_trillionnium_world_bevy_classic_rts_planner_live_autonomous_bot_loop.sh'
  'check_trillionnium_world_bevy_classic_rts_bot_planner_action_executor.sh'
  'check_trillionnium_world_bevy_classic_rts_bot_planner_executor_replay_determinism.sh'
  'check_trillionnium_world_bevy_classic_rts_multi_match_bot_executor_evaluation.sh'
  'check_trillionnium_world_bevy_classic_rts_bot_executor_failure_recovery_matrix.sh'
  'TRNM_MULTI_MATCH_BOT_EXECUTOR_EVALUATION_SUMMARY'
  'TRNM_MULTI_MATCH_BOT_EXECUTOR_EVALUATION_DIR'
  'check_trillionnium_world_bevy_classic_rts_bot_decision_state_gap.sh'
  'check_trillionnium_world_bevy_classic_rts_bot_adaptive_build_order_gap.sh'
  'check_trillionnium_world_bevy_classic_rts_bot_tactical_micro_gap.sh'
  'check_trillionnium_world_bevy_classic_rts_bot_map_intel_gap.sh'
  'check_trillionnium_world_bevy_classic_playtest_readiness.sh'
  'check_trillionnium_world_bevy_classic_playtest_runner_status.sh'
  'refresh_bevy_playtest_runner_for_fast_gates'
  'systemctl --user start trillionnium-bevy-playtest.service'
  'bevy_classic_playtest_runner_status_desktop_fast_gate_refresh'
  'bevy_classic_playtest_runner_status_packet_integrity_refresh'
  'check_trillionnium_world_bevy_classic_playtest_launcher.sh'
  'check_trillionnium_world_bevy_desktop_playtest_review_packet.sh'
  'bevy_desktop_playtest_review_packet_script_contract_guard_test.sh'
  'bevy_desktop_playtest_review_packet_contract_guard'
  'bevy_desktop_playtest_review_packet_gate'
  'check_trillionnium_world_bevy_classic_rts_production_desktop_review_packet.sh'
  'bevy_classic_rts_production_desktop_review_packet_script_contract_guard_test.sh'
  'bevy_classic_rts_production_desktop_review_packet_contract_guard'
  'bevy_classic_rts_production_desktop_review_packet_gate'
  'trillionnium_world_bevy_classic_rts_production_desktop_review_packet_v1'
  'bevy_classic_asset_pack_gate'
  'bevy_classic_manifest_lint_gate'
  'bevy_classic_animation_evidence_contract_guard'
  'bevy_classic_animation_evidence_script_contract_guard_test.sh'
  'bevy_classic_animation_preview_gate'
  'bevy_classic_animation_selector_gate'
  'bevy_classic_player_motion_probe_contract_guard'
  'bevy_classic_player_motion_probe_script_contract_guard_test.sh'
  'bevy_classic_player_motion_probe_gate'
  'bevy_classic_input_frame_budget_contract_guard'
  'bevy_classic_input_frame_budget_gate'
  'bevy_classic_input_frame_budget_script_contract_guard_test.sh'
  'bevy_classic_render_budget_gate'
  'bevy_classic_visual_foundation_contract_guard'
  'bevy_classic_visual_foundation_script_contract_guard_test.sh'
  'bevy_classic_scene_preview_gate'
  'bevy_classic_model_catalog_gate'
  'bevy_classic_renderer_probe_gate'
  'bevy_classic_isometric_modeling_contract_guard'
  'bevy_classic_isometric_modeling_gate'
  'bevy_classic_isometric_modeling_script_contract_guard_test.sh'
  'bevy_classic_rts_openra_like_core_gate'
  'bevy_classic_rts_control_loop_contract_guard'
  'bevy_classic_rts_control_loop_gate'
  'bevy_classic_rts_control_loop_script_contract_guard_test.sh'
  'bevy_classic_rts_selection_minimap_contract_guard'
  'bevy_classic_rts_selection_minimap_gate'
  'bevy_classic_rts_selection_minimap_script_contract_guard_test.sh'
  'bevy_classic_rts_build_lifecycle_contract_guard'
  'bevy_classic_rts_build_lifecycle_gate'
  'bevy_classic_rts_build_lifecycle_script_contract_guard_test.sh'
  'bevy_classic_rts_tech_tree_contract_guard'
  'bevy_classic_rts_tech_tree_gate'
  'bevy_classic_rts_tech_tree_script_contract_guard_test.sh'
  'bevy_classic_rts_projectile_ability_contract_guard'
  'bevy_classic_rts_projectile_ability_gate'
  'bevy_classic_rts_projectile_ability_script_contract_guard_test.sh'
  'bevy_classic_rts_ai_skirmish_pressure_contract_guard'
  'bevy_classic_rts_ai_skirmish_pressure_gate'
  'bevy_classic_rts_ai_skirmish_pressure_script_contract_guard_test.sh'
  'bevy_classic_rts_objective_victory_loop_contract_guard'
  'bevy_classic_rts_objective_victory_loop_gate'
  'bevy_classic_rts_objective_victory_loop_script_contract_guard_test.sh'
  'bevy_classic_rts_creep_camp_terrain_route_contract_guard'
  'bevy_classic_rts_creep_camp_terrain_route_gate'
  'bevy_classic_rts_creep_camp_terrain_route_script_contract_guard_test.sh'
  'bevy_classic_rts_fog_scouting_intel_contract_guard'
  'bevy_classic_rts_fog_scouting_intel_gate'
  'bevy_classic_rts_fog_scouting_intel_script_contract_guard_test.sh'
  'bevy_classic_rts_enemy_base_tech_pressure_contract_guard'
  'bevy_classic_rts_enemy_base_tech_pressure_gate'
  'bevy_classic_rts_enemy_base_tech_pressure_script_contract_guard_test.sh'
  'bevy_classic_rts_army_production_rally_contract_guard'
  'bevy_classic_rts_army_production_rally_gate'
  'bevy_classic_rts_army_production_rally_script_contract_guard_test.sh'
  'bevy_classic_rts_base_assault_resolution_contract_guard'
  'bevy_classic_rts_base_assault_resolution_gate'
  'bevy_classic_rts_base_assault_resolution_script_contract_guard_test.sh'
  'bevy_classic_rts_battle_aftermath_contract_guard'
  'bevy_classic_rts_battle_aftermath_gate'
  'bevy_classic_rts_battle_aftermath_script_contract_guard_test.sh'
  'bevy_classic_rts_commander_progression_contract_guard'
  'bevy_classic_rts_commander_progression_gate'
  'bevy_classic_rts_commander_progression_script_contract_guard_test.sh'
  'bevy_classic_rts_expansion_counterattack_contract_guard'
  'bevy_classic_rts_expansion_counterattack_gate'
  'bevy_classic_rts_expansion_counterattack_script_contract_guard_test.sh'
  'bevy_classic_rts_tier_two_siege_push_contract_guard'
  'bevy_classic_rts_tier_two_siege_push_gate'
  'bevy_classic_rts_tier_two_siege_push_script_contract_guard_test.sh'
  'bevy_classic_rts_siege_breach_counterplay_contract_guard'
  'bevy_classic_rts_siege_breach_counterplay_gate'
  'bevy_classic_rts_siege_breach_counterplay_script_contract_guard_test.sh'
  'bevy_classic_rts_inner_lane_breakthrough_contract_guard'
  'bevy_classic_rts_inner_lane_breakthrough_gate'
  'bevy_classic_rts_inner_lane_breakthrough_script_contract_guard_test.sh'
  'bevy_classic_rts_central_keep_pressure_contract_guard'
  'bevy_classic_rts_central_keep_pressure_gate'
  'bevy_classic_rts_central_keep_pressure_script_contract_guard_test.sh'
  'bevy_classic_rts_central_keep_breakthrough_contract_guard'
  'bevy_classic_rts_central_keep_breakthrough_gate'
  'bevy_classic_rts_central_keep_breakthrough_script_contract_guard_test.sh'
  'bevy_classic_rts_mirror_city_restoration_contract_guard'
  'bevy_classic_rts_mirror_city_restoration_gate'
  'bevy_classic_rts_mirror_city_restoration_script_contract_guard_test.sh'
  'bevy_classic_rts_open_world_after_action_contract_guard'
  'bevy_classic_rts_open_world_after_action_gate'
  'bevy_classic_rts_open_world_after_action_script_contract_guard_test.sh'
  'bevy_classic_rts_campaign_handoff_contract_guard'
  'bevy_classic_rts_campaign_handoff_gate'
  'bevy_classic_rts_campaign_handoff_script_contract_guard_test.sh'
  'bevy_classic_rts_campaign_entry_contract_guard'
  'bevy_classic_rts_campaign_entry_gate'
  'bevy_classic_rts_campaign_entry_script_contract_guard_test.sh'
  'bevy_classic_rts_match_setup_ui_replication_contract_guard'
  'bevy_classic_rts_match_setup_ui_replication_gate'
  'bevy_classic_rts_match_setup_ui_replication_script_contract_guard_test.sh'
  'bevy_classic_rts_campaign_outcome_ui_readiness_contract_guard'
  'bevy_classic_rts_campaign_outcome_ui_readiness_gate'
  'bevy_classic_rts_campaign_outcome_ui_readiness_script_contract_guard_test.sh'
  'bevy_classic_rts_campaign_ui_continuity_contract_guard'
  'bevy_classic_rts_campaign_ui_continuity_gate'
  'bevy_classic_rts_campaign_ui_continuity_script_contract_guard_test.sh'
  'bevy_classic_rts_in_match_hud_state_replication_contract_guard'
  'bevy_classic_rts_in_match_hud_state_replication_gate'
  'bevy_classic_rts_in_match_hud_state_replication_script_contract_guard_test.sh'
  'bevy_classic_rts_session_state_continuity_contract_guard'
  'bevy_classic_rts_session_state_continuity_gate'
  'bevy_classic_rts_session_state_continuity_script_contract_guard_test.sh'
  'bevy_classic_rts_continuous_player_flow_contract_guard'
  'bevy_classic_rts_continuous_player_flow_gate'
  'bevy_classic_rts_continuous_player_flow_script_contract_guard_test.sh'
  'bevy_classic_rts_live_session_playthrough_contract_guard'
  'bevy_classic_rts_live_session_playthrough_gate'
  'bevy_classic_rts_live_session_playthrough_script_contract_guard_test.sh'
  'bevy_classic_rts_visual_fidelity_contract_guard'
  'bevy_classic_rts_visual_fidelity_gate'
  'bevy_classic_rts_visual_fidelity_script_contract_guard_test.sh'
  'bevy_classic_rts_production_art_replication_contract_guard'
  'bevy_classic_rts_production_art_replication_gate'
  'bevy_classic_rts_production_art_replication_script_contract_guard_test.sh'
  'bevy_classic_rts_production_asset_atlas_contract_guard'
  'bevy_classic_rts_production_asset_atlas_gate'
  'bevy_classic_rts_production_asset_atlas_script_contract_guard_test.sh'
  'bevy_classic_rts_production_ui_skin_contract_guard'
  'bevy_classic_rts_production_ui_skin_gate'
  'bevy_classic_rts_production_ui_skin_script_contract_guard_test.sh'
  'bevy_classic_rts_production_interaction_polish_contract_guard'
  'bevy_classic_rts_production_interaction_polish_gate'
  'bevy_classic_rts_production_interaction_polish_script_contract_guard_test.sh'
  'bevy_classic_rts_full_screen_ui_replication_contract_guard'
  'bevy_classic_rts_full_screen_ui_replication_gate'
  'bevy_classic_rts_full_screen_ui_replication_script_contract_guard_test.sh'
  'bevy_classic_rts_shell_meta_ui_replication_contract_guard'
  'bevy_classic_rts_shell_meta_ui_replication_gate'
  'bevy_classic_rts_shell_meta_ui_replication_script_contract_guard_test.sh'
  'bevy_classic_rts_command_affordance_contract_guard'
  'bevy_classic_rts_command_affordance_gate'
  'bevy_classic_rts_command_affordance_script_contract_guard_test.sh'
  'bevy_classic_rts_action_cadence_contract_guard'
  'bevy_classic_rts_action_cadence_gate'
  'bevy_classic_rts_action_cadence_script_contract_guard_test.sh'
  'bevy_classic_rts_unit_model_depth_contract_guard'
  'bevy_classic_rts_unit_model_depth_gate'
  'bevy_classic_rts_unit_model_depth_script_contract_guard_test.sh'
  'bevy_classic_rts_action_sequence_contract_guard'
  'bevy_classic_rts_action_sequence_gate'
  'bevy_classic_rts_action_sequence_script_contract_guard_test.sh'
  'bevy_classic_rts_npc_behavior_contract_guard'
  'bevy_classic_rts_npc_behavior_gate'
  'bevy_classic_rts_npc_behavior_script_contract_guard_test.sh'
  'bevy_classic_rts_combat_impact_contract_guard'
  'bevy_classic_rts_combat_impact_gate'
  'bevy_classic_rts_combat_impact_script_contract_guard_test.sh'
  'bevy_classic_rts_locomotion_blend_contract_guard'
  'bevy_classic_rts_locomotion_blend_gate'
  'bevy_classic_rts_locomotion_blend_script_contract_guard_test.sh'
  'bevy_classic_rts_npc_transition_contract_guard'
  'bevy_classic_rts_npc_transition_gate'
  'bevy_classic_rts_npc_transition_script_contract_guard_test.sh'
  'bevy_classic_rts_depth_readability_contract_guard'
  'bevy_classic_rts_depth_readability_gate'
  'bevy_classic_rts_depth_readability_script_contract_guard_test.sh'
  'bevy_classic_rts_combat_readability_pressure_readiness_contract_guard'
  'bevy_classic_rts_combat_readability_pressure_readiness_gate'
  'bevy_classic_rts_combat_readability_pressure_readiness_script_contract_guard_test.sh'
  'bevy_classic_rts_openra_parity_bridge_contract_guard'
  'bevy_classic_rts_openra_parity_bridge_gate'
  'bevy_classic_rts_openra_parity_bridge_script_contract_guard_test.sh'
  'bevy_classic_rts_owned_replay_file_contract_guard'
  'bevy_classic_rts_owned_replay_file_gate'
  'bevy_classic_rts_owned_replay_file_script_contract_guard_test.sh'
  'bevy_classic_rts_headless_replay_playback_contract_guard'
  'bevy_classic_rts_headless_replay_playback_gate'
  'bevy_classic_rts_headless_replay_playback_script_contract_guard_test.sh'
  'bevy_classic_rts_natural_terminal_contract_contract_guard'
  'bevy_classic_rts_natural_terminal_contract_gate'
  'bevy_classic_rts_natural_terminal_contract_script_contract_guard_test.sh'
  'bevy_classic_rts_native_bot_ai_planner_contract_guard'
  'bevy_classic_rts_native_bot_ai_planner_gate'
  'bevy_classic_rts_native_bot_ai_planner_script_contract_guard_test.sh'
  'bevy_classic_rts_planner_live_autonomous_bot_loop_contract_guard'
  'bevy_classic_rts_planner_live_autonomous_bot_loop_gate'
  'bevy_classic_rts_planner_live_autonomous_bot_loop_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_planner_action_executor_contract_guard'
  'bevy_classic_rts_bot_planner_action_executor_gate'
  'bevy_classic_rts_bot_planner_action_executor_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_planner_executor_replay_determinism_contract_guard'
  'bevy_classic_rts_bot_planner_executor_replay_determinism_gate'
  'bevy_classic_rts_bot_planner_executor_replay_determinism_script_contract_guard_test.sh'
  'bevy_classic_rts_multi_match_bot_executor_evaluation_contract_guard'
  'bevy_classic_rts_multi_match_bot_executor_evaluation_gate'
  'bevy_classic_rts_multi_match_bot_executor_evaluation_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_executor_failure_recovery_matrix_contract_guard'
  'bevy_classic_rts_bot_executor_failure_recovery_matrix_gate'
  'bevy_classic_rts_bot_executor_failure_recovery_matrix_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_decision_state_gap_contract_guard'
  'bevy_classic_rts_bot_decision_state_gap_gate'
  'bevy_classic_rts_bot_decision_state_gap_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_adaptive_build_order_gap_contract_guard'
  'bevy_classic_rts_bot_adaptive_build_order_gap_gate'
  'bevy_classic_rts_bot_adaptive_build_order_gap_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_tactical_micro_gap_contract_guard'
  'bevy_classic_rts_bot_tactical_micro_gap_gate'
  'bevy_classic_rts_bot_tactical_micro_gap_script_contract_guard_test.sh'
  'bevy_classic_rts_bot_map_intel_gap_contract_guard'
  'bevy_classic_rts_bot_map_intel_gap_gate'
  'bevy_classic_rts_bot_map_intel_gap_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_classic_asset_slot_map.sh'
  'bevy_classic_asset_slot_map_gate'
  'check_trillionnium_world_bevy_classic_art_pack.sh'
  'bevy_classic_art_pack_gate'
  'check_trillionnium_world_bevy_classic_art_pack_scene_probe.sh'
  'bevy_classic_art_pack_scene_probe_gate'
  'check_trillionnium_world_bevy_classic_asset_override_probe.sh'
  'bevy_classic_asset_override_probe_gate'
  'check_trillionnium_world_bevy_classic_rts_live_input_sequence.sh'
  'bevy_classic_rts_live_input_sequence_gate'
  'check_trillionnium_world_bevy_classic_rts_pathing_formation.sh'
  'bevy_classic_rts_pathing_formation_gate'
  'check_trillionnium_world_bevy_classic_rts_collision_engagement.sh'
  'bevy_classic_rts_collision_engagement_gate'
  'check_trillionnium_world_bevy_classic_rts_target_aggro_focus.sh'
  'bevy_classic_rts_target_aggro_focus_gate'
  'check_trillionnium_world_bevy_classic_rts_economy_build.sh'
  'bevy_classic_rts_economy_build_gate'
  'check_trillionnium_world_bevy_classic_rts_autonomous_bot_skirmish.sh'
  'bevy_classic_rts_autonomous_bot_skirmish_gate'
  'check_trillionnium_world_bevy_classic_rts_organic_terminal_gap.sh'
  'bevy_classic_rts_organic_terminal_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_terminal_observation_gap.sh'
  'bevy_classic_rts_terminal_observation_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_replay_metrics_gap.sh'
  'bevy_classic_rts_replay_metrics_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_endurance_skirmish_gap.sh'
  'bevy_classic_rts_endurance_skirmish_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_bot_macro_economy_gap.sh'
  'bevy_classic_rts_bot_macro_economy_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_bot_harassment_defense_gap.sh'
  'bevy_classic_rts_bot_harassment_defense_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_bot_multi_front_pressure_gap.sh'
  'bevy_classic_rts_bot_multi_front_pressure_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_bot_expansion_control_gap.sh'
  'bevy_classic_rts_bot_expansion_control_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_bot_tech_transition_gap.sh'
  'bevy_classic_rts_bot_tech_transition_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_bot_army_composition_gap.sh'
  'bevy_classic_rts_bot_army_composition_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_map_model_gap.sh'
  'bevy_classic_rts_map_model_gap_gate'
  'check_trillionnium_world_bevy_classic_rts_first_minute_readiness.sh'
  'bevy_classic_rts_first_minute_readiness_gate'
  'check_trillionnium_world_bevy_classic_rts_map_ui_modeling_readiness.sh'
  'bevy_classic_rts_map_ui_modeling_readiness_gate'
  'check_trillionnium_world_bevy_classic_rts_playtest_observability_readiness.sh'
  'bevy_classic_rts_playtest_observability_readiness_gate'
  'bevy_classic_rts_command_surface_contract_guard'
  'bevy_classic_rts_command_surface_gate'
  'bevy_classic_rts_command_surface_script_contract_guard_test.sh'
  'bevy_classic_rts_structure_modeling_contract_guard'
  'bevy_classic_rts_structure_modeling_gate'
  'bevy_classic_rts_structure_modeling_script_contract_guard_test.sh'
  'bevy_classic_rts_environment_life_contract_guard'
  'bevy_classic_rts_environment_life_gate'
  'bevy_classic_rts_environment_life_script_contract_guard_test.sh'
  'bevy_classic_rts_worker_harvest_animation_contract_guard'
  'bevy_classic_rts_worker_harvest_animation_gate'
  'bevy_classic_rts_worker_harvest_animation_script_contract_guard_test.sh'
  'bevy_classic_rts_ability_tooltip_telegraph_contract_guard'
  'bevy_classic_rts_ability_tooltip_telegraph_gate'
  'bevy_classic_rts_ability_tooltip_telegraph_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_hotkey_feedback_contract_guard'
  'bevy_classic_rts_control_group_hotkey_feedback_gate'
  'bevy_classic_rts_control_group_hotkey_feedback_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_recall_formation_preview_contract_guard'
  'bevy_classic_rts_control_group_recall_formation_preview_gate'
  'bevy_classic_rts_control_group_recall_formation_preview_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_recall_override_preview_contract_guard'
  'bevy_classic_rts_control_group_recall_override_preview_gate'
  'bevy_classic_rts_control_group_recall_override_preview_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_command_feedback_strip_contract_guard'
  'bevy_classic_rts_control_group_command_feedback_strip_gate'
  'bevy_classic_rts_control_group_command_feedback_strip_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_command_feedback_lifecycle_contract_guard'
  'bevy_classic_rts_control_group_command_feedback_lifecycle_gate'
  'bevy_classic_rts_control_group_command_feedback_lifecycle_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_command_history_contract_guard'
  'bevy_classic_rts_control_group_command_history_gate'
  'bevy_classic_rts_control_group_command_history_script_contract_guard_test.sh'
  'bevy_classic_rts_control_group_command_history_prune_contract_guard'
  'bevy_classic_rts_control_group_command_history_prune_gate'
  'bevy_classic_rts_control_group_command_history_prune_script_contract_guard_test.sh'
  'bevy_classic_rts_scrollable_map_contract_guard'
  'bevy_classic_rts_scrollable_map_gate'
  'bevy_classic_rts_scrollable_map_script_contract_guard_test.sh'
  'bevy_classic_rts_camera_minimap_sync_contract_guard'
  'bevy_classic_rts_camera_minimap_sync_gate'
  'bevy_classic_rts_camera_minimap_sync_script_contract_guard_test.sh'
  'bevy_classic_playtest_readiness_gate "$ROOT/scripts/check_trillionnium_world_bevy_classic_playtest_readiness.sh" --no-refresh'
  'bevy_classic_playtest_readiness_gate'
  'bevy_classic_playtest_runner_status_contract_guard'
  'bevy_classic_playtest_runner_status_script_contract_guard_test.sh'
  'bevy_classic_playtest_launcher_contract_guard'
  'bevy_classic_playtest_launcher_gate'
  'bevy_classic_playtest_launcher_script_contract_guard_test.sh'
  'bevy_classic_playtest_readiness_refresh_mode_script_contract_guard_test.sh'
  'trillionnium_world_bevy_classic_asset_pack_v1'
  'trillionnium_world_bevy_classic_manifest_lint_v1'
  'trillionnium_world_bevy_classic_animation_preview_v1'
  'trillionnium_world_bevy_classic_animation_selector_v1'
  'trillionnium_world_bevy_classic_player_motion_probe_v1'
  'trillionnium_world_bevy_classic_input_frame_budget_v1'
  'trillionnium_world_bevy_classic_render_budget_v1'
  'trillionnium_world_bevy_classic_scene_preview_v1'
  'trillionnium_world_bevy_classic_model_catalog_v1'
  'trillionnium_world_bevy_classic_renderer_probe_v1'
  'trillionnium_world_bevy_classic_isometric_modeling_v1'
  'trillionnium_world_bevy_classic_rts_control_loop_v1'
  'trillionnium_world_bevy_classic_rts_selection_minimap_v1'
  'trillionnium_world_bevy_classic_rts_build_lifecycle_v1'
  'trillionnium_world_bevy_classic_rts_tech_tree_v1'
  'trillionnium_world_bevy_classic_rts_projectile_ability_v1'
  'trillionnium_world_bevy_classic_rts_ai_skirmish_pressure_v1'
  'trillionnium_world_bevy_classic_rts_objective_victory_loop_v1'
  'trillionnium_world_bevy_classic_rts_creep_camp_terrain_route_v1'
  'trillionnium_world_bevy_classic_rts_fog_scouting_intel_v1'
  'trillionnium_world_bevy_classic_rts_enemy_base_tech_pressure_v1'
  'trillionnium_world_bevy_classic_rts_army_production_rally_v1'
  'trillionnium_world_bevy_classic_rts_base_assault_resolution_v1'
  'trillionnium_world_bevy_classic_rts_battle_aftermath_v1'
  'trillionnium_world_bevy_classic_rts_commander_progression_v1'
  'trillionnium_world_bevy_classic_rts_expansion_counterattack_v1'
  'trillionnium_world_bevy_classic_rts_tier_two_siege_push_v1'
  'trillionnium_world_bevy_classic_rts_siege_breach_counterplay_v1'
  'trillionnium_world_bevy_classic_rts_inner_lane_breakthrough_v1'
  'trillionnium_world_bevy_classic_rts_central_keep_pressure_v1'
  'trillionnium_world_bevy_classic_rts_central_keep_breakthrough_v1'
  'trillionnium_world_bevy_classic_rts_mirror_city_restoration_v1'
  'trillionnium_world_bevy_classic_rts_open_world_after_action_v1'
  'trillionnium_world_bevy_classic_rts_campaign_handoff_v1'
  'trillionnium_world_bevy_classic_rts_campaign_entry_v1'
  'trillionnium_world_bevy_classic_rts_match_setup_ui_replication_v1'
  'trillionnium_world_bevy_classic_rts_campaign_outcome_ui_readiness_v1'
  'trillionnium_world_bevy_classic_rts_campaign_ui_continuity_v1'
  'trillionnium_world_bevy_classic_rts_in_match_hud_state_replication_v1'
  'trillionnium_world_bevy_classic_rts_session_state_continuity_v1'
  'trillionnium_world_bevy_classic_rts_visual_fidelity_v1'
  'trillionnium_world_bevy_classic_rts_command_affordance_v1'
  'trillionnium_world_bevy_classic_rts_action_cadence_v1'
  'trillionnium_world_bevy_classic_rts_unit_model_depth_v1'
  'trillionnium_world_bevy_classic_rts_action_sequence_v1'
  'trillionnium_world_bevy_classic_rts_npc_behavior_loop_v1'
  'trillionnium_world_bevy_classic_rts_combat_impact_loop_v1'
  'trillionnium_world_bevy_classic_rts_locomotion_blend_v1'
  'trillionnium_world_bevy_classic_rts_npc_transition_blend_v1'
  'trillionnium_world_bevy_classic_rts_command_surface_v1'
  'trillionnium_world_bevy_classic_rts_structure_modeling_v1'
  'trillionnium_world_bevy_classic_rts_environment_life_v1'
  'trillionnium_world_bevy_classic_rts_worker_harvest_animation_v1'
  'trillionnium_world_bevy_classic_rts_ability_tooltip_telegraph_v1'
  'trillionnium_world_bevy_classic_rts_control_group_hotkey_feedback_v1'
  'trillionnium_world_bevy_classic_rts_control_group_recall_formation_preview_v1'
  'trillionnium_world_bevy_classic_rts_control_group_recall_override_preview_v1'
  'trillionnium_world_bevy_classic_rts_control_group_command_feedback_strip_v1'
  'trillionnium_world_bevy_classic_rts_control_group_command_feedback_lifecycle_v1'
  'trillionnium_world_bevy_classic_rts_control_group_command_history_v1'
  'trillionnium_world_bevy_classic_rts_control_group_command_history_prune_v1'
  'trillionnium_world_bevy_classic_rts_scrollable_map_v1'
  'trillionnium_world_bevy_classic_rts_camera_minimap_sync_v1'
  'trillionnium_world_bevy_classic_rts_depth_readability_v1'
  'trillionnium_world_bevy_classic_rts_combat_readability_pressure_readiness_v1'
  'trillionnium_world_bevy_classic_rts_openra_parity_bridge_v1'
  'trillionnium_world_bevy_classic_rts_owned_replay_file_v1'
  'trillionnium_world_bevy_classic_rts_headless_replay_playback_v1'
  'trillionnium_world_bevy_classic_rts_natural_terminal_contract_v1'
  'trillionnium_world_bevy_classic_rts_native_bot_ai_planner_v1'
  'trillionnium_world_bevy_classic_rts_planner_live_autonomous_bot_loop_v1'
  'trillionnium_world_bevy_classic_rts_bot_planner_action_executor_v1'
  'trillionnium_world_bevy_classic_rts_bot_planner_executor_replay_determinism_v1'
  'trillionnium_world_bevy_classic_rts_multi_match_bot_executor_evaluation_v1'
  'trillionnium_world_bevy_classic_rts_bot_executor_failure_recovery_matrix_v1'
  'trillionnium_world_bevy_classic_rts_bot_decision_state_gap_v1'
  'trillionnium_world_bevy_classic_rts_bot_adaptive_build_order_gap_v1'
  'trillionnium_world_bevy_classic_rts_bot_tactical_micro_gap_v1'
  'trillionnium_world_bevy_classic_rts_bot_map_intel_gap_v1'
  'trillionnium_world_bevy_classic_playtest_readiness_v1'
  'trillionnium_world_bevy_classic_rts_production_asset_atlas_v1'
  'trillionnium_world_bevy_classic_rts_production_ui_skin_v1'
  'trillionnium_world_bevy_classic_rts_production_interaction_polish_v1'
  'trillionnium_world_bevy_classic_rts_full_screen_ui_replication_v1'
  'trillionnium_world_bevy_classic_rts_shell_meta_ui_replication_v1'
  'trillionnium_world_bevy_classic_playtest_runner_status_v1'
  'trillionnium_world_bevy_classic_playtest_launcher_v1'
  'action_coach_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_action_coach.sh'
  'trillionnium_world_bevy_action_coach_v1'
  'bevy_action_coach_contract_guard'
  'bevy_action_coach_gate'
  'authored_art_pack_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_art_pack.sh'
  'authored_sprite_sheet_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_sprite_sheet.sh'
  'authored_texture_atlas_binding_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_texture_atlas_binding.sh'
  'authored_material_consumption_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_material_consumption.sh'
  'authored_material_application_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_material_application.sh'
  'runtime_texture_asset_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_runtime_texture_asset.sh'
  'runtime_texture_manifest_probe_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_runtime_texture_manifest_probe.sh'
  'asset_store_registration_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_asset_store_registration.sh'
  'sprite_asset_binding_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_sprite_asset_binding.sh'
  'sprite_texture_sampling_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_sprite_texture_sampling.sh'
  'authored_render_frame_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_render_frame.sh'
  'authored_live_visual_bridge_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_authored_live_visual_bridge.sh'
  'bevy_live_window_artifact_wrapper_script_contract_guard_test.sh'
  'bevy_live_window_artifact_wrapper_contract_guard'
  'live_window_layer_pixel_probe_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_live_window_layer_pixel_probe.sh'
  'live_window_texture_correlation_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_live_window_texture_correlation.sh'
  'live_window_sampled_texture_correlation_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_live_window_sampled_texture_correlation.sh'
  'render_asset_eligibility_script_contract_guard_test.sh'
  'check_trillionnium_world_bevy_render_asset_eligibility.sh'
  'live_window_runtime_texture_manifest_script_contract_guard_test.sh'
  'bevy_live_window_screenshot_sequence_gate'
  'check_trillionnium_world_bevy_live_window_screenshot_sequence.sh'
  'TRNM_LIVE_WINDOW_RUNTIME_TEXTURE_ASSET_SUMMARY="$ROOT/acceptance/S5_native_bevy_device/latest/bevy-runtime-texture-asset.json"'
  'TRNM_LIVE_WINDOW_RUNTIME_TEXTURE_ASSET_MANIFEST="$ROOT/acceptance/S5_native_bevy_device/latest/bevy-runtime-texture-asset-manifest.json"'
  'bevy_live_window_screenshot_sequence_artifact'
  'capture_diagnostics_gate'
  'capture_attempt_counts'
  'trillionnium_world_bevy_runtime_texture_asset_v1'
  'trillionnium_world_bevy_runtime_texture_manifest_probe_v1'
  'bevy_runtime_texture_manifest_probe_gate'
  'trillionnium_world_bevy_asset_store_registration_v1'
  'bevy_asset_store_registration_gate'
  'trillionnium_world_bevy_sprite_asset_binding_v1'
  'bevy_sprite_asset_binding_gate'
  'trillionnium_world_bevy_sprite_texture_sampling_v1'
  'bevy_sprite_texture_sampling_gate'
  'trillionnium_world_bevy_live_window_sampled_texture_correlation_v1'
  'bevy_live_window_sampled_texture_correlation_gate'
  'trillionnium_world_bevy_render_asset_eligibility_v1'
  'bevy_render_asset_eligibility_gate'
  'runtime_texture_sprite_asset_binding_gate'
  'runtime_texture_sprite_bound_surface_count'
  'runtime_texture_manifest_hash_gate'
  'runtime_texture_launch_env_gate'
  'runtime_texture_handle_gate'
  'public_launch_evidence_intake_script_contract_guard_test.sh'
  'check_trillionnium_world_public_launch_evidence_intake.sh'
  'check_trillionnium_world_review_slice_manifest.sh'
  'check_trillionnium_world_review_triage_queue.sh'
  'check_trillionnium_world_review_primary_owner_plan.sh'
  'check_trillionnium_world_review_release_owner_queue.sh'
  'check_trillionnium_world_review_runtime_owner_queue.sh'
  'check_trillionnium_world_review_public_boundary_batch.sh'
  'check_trillionnium_world_review_release_native_handoff_batch.sh'
  'check_trillionnium_world_review_runtime_boundary_batch.sh'
  'check_trillionnium_world_review_runtime_core_semantics_batch.sh'
  'check_trillionnium_world_review_runtime_adapter_online_batch.sh'
  'check_trillionnium_world_review_openra_parity_claim_batch.sh'
  'check_trillionnium_world_review_first_contact_rts_data_batch.sh'
  'check_trillionnium_world_review_rts_evidence_crate_batch.sh'
  'check_trillionnium_world_review_evidence_exposure_batch.sh'
  'check_trillionnium_world_review_bevy_runtime_renderer_batch.sh'
  'check_trillionnium_world_review_first_contact_player_surface_cues_batch.sh'
  'check_trillionnium_world_review_generated_count_surface_batch.sh'
  'check_trillionnium_world_review_docs_plan_truth_source_batch.sh'
  'check_trillionnium_world_review_bot_executor_surface_batch.sh'
  'TRNM_WORLD_REVIEW_RELEASE_NATIVE_HANDOFF_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_RUNTIME_BOUNDARY_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_RUNTIME_CORE_SEMANTICS_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_OPENRA_PARITY_CLAIM_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_FIRST_CONTACT_RTS_DATA_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_RTS_EVIDENCE_CRATE_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_EVIDENCE_EXPOSURE_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_BEVY_RUNTIME_RENDERER_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_FIRST_CONTACT_PLAYER_SURFACE_CUES_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_GENERATED_COUNT_SURFACE_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_DOCS_PLAN_TRUTH_SOURCE_BATCH_REFRESH_INPUTS=0'
  'TRNM_WORLD_REVIEW_BOT_EXECUTOR_SURFACE_BATCH_REFRESH_INPUTS=0'
  'world_review_triage_queue'
  'world_review_primary_owner_plan'
  'world_review_release_owner_queue'
  'world_review_runtime_owner_queue'
  'world_review_public_boundary_batch'
  'world_review_release_native_handoff_batch'
  'world_review_runtime_boundary_batch'
  'world_review_runtime_core_semantics_batch'
  'world_review_runtime_adapter_online_batch'
  'world_review_openra_parity_claim_batch'
  'world_review_first_contact_rts_data_batch'
  'world_review_rts_evidence_crate_batch'
  'world_review_evidence_exposure_batch'
  'world_review_bevy_runtime_renderer_batch'
  'world_review_first_contact_player_surface_cues_batch'
  'world_review_generated_count_surface_batch'
  'world_review_docs_plan_truth_source_batch'
  'world_review_bot_executor_surface_batch'
  'release_readiness_release_review_entry_guard_test.sh'
  'production_map_pack_public_evidence_collection_script_contract_guard_test.sh'
  'check_trillionnium_world_production_map_pack_public_evidence_collection.sh'
  'production_map_pack_public_evidence_script_contract_guard_test.sh'
  'map_modeling_gate_script_contract_guard_test.sh'
  'check_trillionnium_world_map_modeling_gate.sh'
  'map_modeling_gate_contract_guard'
  'map_modeling_gate'
  'root_readme_world_release_review_quickcheck_guard_test.sh'
  'check_root_readme_local_links.sh'
  'WORKFLOW_SCRIPT_REF_STRICT=1'
  'validate_workflow_script_refs.sh'
  'release_review_ci_gate_runs_local_bevy_playability_packet_integrity_static_guards_readme_links_workflow_refs_and_checkpoint_manifest_without_claiming_android_s5_real_device_ready'
  'android_s5_real_device_claimed: false'
  'host_side_bevy_runtime_replay_not_android_real_device'
)

for line in "${required_lines[@]}"; do
  if ! grep -Fq -- "$line" "$SCRIPT"; then
    echo "[FAIL] release review CI gate script missing contract line: $line" >&2
    exit 1
  fi
done

if grep -Fq '&& -x "$ROOT/target/release/trnm-world-bevy"' "$SCRIPT"; then
  echo "[FAIL] release review CI gate must refresh the release Bevy binary before exporting it, not trust a stale executable" >&2
  exit 1
fi

duplicate_packet_integrity_guard_runs=(
  'run_check packet_integrity_semantic_guard '
  'run_check packet_integrity_bot_executor_semantic_guard '
  'run_check packet_integrity_bot_executor_matrix_semantic_guard '
  'run_check packet_integrity_bot_gap_semantic_guard '
  'run_check packet_integrity_control_loop_semantic_guard '
  'run_check packet_integrity_selection_minimap_semantic_guard '
  'run_check packet_integrity_build_lifecycle_semantic_guard '
  'run_check packet_integrity_tech_tree_semantic_guard '
  'run_check packet_integrity_projectile_ability_semantic_guard '
)

for line in "${duplicate_packet_integrity_guard_runs[@]}"; do
  if grep -Fq "$line" "$SCRIPT"; then
    echo "[FAIL] release review CI gate reintroduced duplicate packet integrity semantic guard run: $line" >&2
    exit 1
  fi
done

summary_line="$(grep -nF 'run_check packet_integrity_semantic_fixture_summary_refresh_gate' "$SCRIPT" | head -n 1 | cut -d: -f1)"
packet_line="$(grep -nF 'run_check release_packet_refresh_gate' "$SCRIPT" | head -n 1 | cut -d: -f1)"
suite_line="$(grep -nF 'run_check packet_integrity_semantic_fixture_suite_gate' "$SCRIPT" | head -n 1 | cut -d: -f1)"
if [[ -z "$summary_line" || -z "$packet_line" || -z "$suite_line" ]] ||
  (( summary_line >= packet_line || packet_line >= suite_line )); then
  echo "[FAIL] release review CI must refresh semantic fixture summaries, then packet, then reused-packet semantic fixture suite" >&2
  exit 1
fi

echo "[PASS] release review CI gate script keeps packet integrity, static guards, README links, workflow refs, checkpoint manifest, next execution plan, and Android S5 boundary"
