...
required_lines=(
  ...
  'contract_match'
  'status_match'
  'length) == 120 or ((.artifacts // []) | length) == 124 or ((.artifacts // []) | length) == 126'
  'expected one hundred twenty, one hundred twenty-four, or one hundred twenty-six artifacts including live-window mouse hit-test sequence, classic playtest launcher, playtest handoff readiness/packet/Markdown, full screen/UI replication, shell/meta UI replication, match setup UI replication, campaign outcome UI readiness, campaign UI continuity JSON/PPM, in-match HUD/state replication, session state continuity, combat readability/pressure readiness, camera/minimap sync JSON/PPM, full-game visual/UI replication JSON/PPM, OpenRA screen-for-screen UI replication JSON/PPM, OpenRA engine port asset parity JSON/PPM'
