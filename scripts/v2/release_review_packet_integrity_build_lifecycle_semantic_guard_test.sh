...
set -e
if [[ "$status" -eq 0 ]]; then
  echo "[FAIL] packet integrity build lifecycle semantic fixture unexpectedly passed" >&2
  cat "$TMP_DIR/stdout.log" >&2
  cat "$TMP_DIR/stderr.log" >&2
  exit 1
fi
if [[ ! -f "$summary_json" ]]; then
  echo "[FAIL] packet integrity build lifecycle semantic fixture did not write summary" >&2
  exit 1
fi
jq -e '
  ...
  and ([.failures[].name] | index("classic_rts_build_lifecycle_semantics"))
  and ([.failures[].name] | index("classic_rts_build_lifecycle_ppm_semantics"))
...
echo "[PASS] release review packet integrity rejects semantically invalid classic RTS build lifecycle summary and PPM even when checksums match"
