...
def main() -> None:
    ...
    for endpoint in MUTATION_ENDPOINTS:
        if endpoint in source:
            fail(f"mutation endpoint present: {endpoint}")

    if source.count("urllib.request.Request(") != 2:
        fail("only the JSON GET and artifact GET request constructors are permitted")
    if "workflow_runs" not in source or "if not workflow_runs" not in source:
        fail("empty workflow-run collection is not rejected")
    if "if not jobs" not in source or "runner_id != 0" not in source or "bool(steps)" not in source:
        fail("non-empty runner/job/step evidence is not enforced")
    if "if not retained" not in source or "bound_manifest" not in source:
        fail("artifact and manifest evidence is not enforced")
    if "if not approvals" not in source:
        fail("independent approval is not enforced")

    print("TRNM_WORLD_CEX_SEQUENCE_50_VERIFIER_CONTRACT_V2=PASS")
