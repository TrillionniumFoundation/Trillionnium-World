...
add_item s5_android_real_device_matrix s5_real_device_matrix "S5 Android real-device matrix" ANDROID_SERIAL "$S5_DIR/s5-device-evidence.template.json" "TRILLIONNIUM_WORLD_S5_REAL_DEVICE_EVIDENCE_PATH=<real-s5-evidence.json> scripts/check_trillionnium_world_s5_real_device_evidence.sh --require-ready" "$S5_SUMMARY" s5_real_device_evidence_green "$(read_json_field "$S5_SUMMARY" '.status')" "Attach a USB-debugging Android device, confirm adb shows it as device, run scripts/check_trillionnium_world_s5_device_evidence.sh --require-device, then validate the generated real Android serial, adb devices output, screenshot, gfxinfo/frame stats, logcat, lifecycle, crash-free window, APK, native library, and symbol evidence."
