...
# Match actual forbidden commands/keys while allowing `git commit-tree`, which
# creates one unreachable prospective-merge object without moving a ref,
# persisting credentials, or modifying candidate source.
FORBIDDEN = (
...
WORKFLOW_JOBS = {
    "trnm-world-authority-cutover.yml": {
        "authority-source-qualification": "trnm-world-p0-boundaries",
        "authority-status-evidence": "trnm-world-status-evidence",
    },
...
def workflow_inventory(root: pathlib.Path) -> dict[str, str]:
    ...
    if {path.name for path in files} != set(WORKFLOW_JOBS):
        fail("active workflow inventory differs from the reviewed ten files")
...
def main() -> int:
    ...
    try:
        ...
        fail(str(error))
    scope = "workflow inventory only" if args.workflows_only else "workflow inventory, current documentation and historical schema"
