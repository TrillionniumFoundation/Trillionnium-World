...
DOC_REL="docs/archive/world-review-2026-07/trillionnium-world-review-release-owner-queue-2026-07-07.md"
DOC="$ROOT/$DOC_REL"
OWNER_PLAN_JSON="$ACCEPTANCE_DIR/trillionnium-world-review-primary-owner-plan.json"
SUMMARY="$ACCEPTANCE_DIR/trillionnium-world-review-release-owner-queue.json"
SUMMARY_MD="$ACCEPTANCE_DIR/trillionnium-world-review-release-owner-queue.md"
...
bucket_queue_role() {
  case "$1" in
    multi_public_boundary_overlap)
      printf 'Review public/external evidence boundaries before product/runtime details.'
      ;;
    multi_release_native_handoff_overlap)
      printf 'Review release truth and no-credit handoff language before playable-client details.'
      ;;
    unclassified_generated_count_surface)
      printf 'Confirm each generated count is owned by a checker/artifact contract.'
      ;;
    unclassified_docs_plan_truth_source)
      printf 'Confirm each doc is current truth or route it to archive/reference-only.'
      ;;
    *)
      printf 'Not part of the release/public-boundary owner queue.'
      ;;
  esac
}
...
bucket_review_order() {
  case "$1" in
    multi_public_boundary_overlap) printf '1' ;;
    multi_release_native_handoff_overlap) printf '2' ;;
    unclassified_generated_count_surface) printf '4' ;;
    unclassified_docs_plan_truth_source) printf '5' ;;
    *) printf '99' ;;
  esac
}

bucket_requires_commit_level_owner() {
  case "$1" in
    multi_public_boundary_overlap|multi_release_native_handoff_overlap) printf 'true' ;;
    *) printf 'false' ;;
  esac
}

is_release_owner_bucket() {
  case "$1" in
    multi_public_boundary_overlap|multi_release_native_handoff_overlap|unclassified_generated_count_surface|unclassified_docs_plan_truth_source)
      return 0
      ;;
    *)
      return 1
...
if [[ ! -f "$DOC" ]]; then
  echo "[FAIL] missing review release-owner queue doc: $DOC" >&2
  exit 1
fi

require_text "$DOC" "Status: local release/public-boundary owner queue."
...
queue_jsonl="$TMP_DIR/release_owner_queue.jsonl"
: >"$queue_jsonl"
queue_order=0
...
while IFS=$'\t' read -r hash subject; do
  ...
  if ! is_release_owner_bucket "$bucket"; then
    continue
  fi
  ...
  jq -n \
    ...
    --argjson commit_level_primary_owner_review_required "$(bucket_requires_commit_level_owner "$bucket")" \
    ...
    '{
      ...
      commit_level_primary_owner_review_required: $commit_level_primary_owner_review_required,
...
queue_items_json="$(jq -s 'sort_by(.owner_review_order, .queue_order)' "$queue_jsonl")"
bucket_counts_json="$(
  jq -n --argjson items "$queue_items_json" '
    [
      "multi_public_boundary_overlap",
      "multi_release_native_handoff_overlap",
      "unclassified_generated_count_surface",
    ...
    | map(. as $id | {
        ...
        owner_review_order: (
          if $id == "multi_public_boundary_overlap" then 1
          elif $id == "multi_release_native_handoff_overlap" then 2
          elif $id == "unclassified_generated_count_surface" then 4
          else 5
          end
        ),
        queue_item_count: ([ $items[] | select(.bucket_id == $id) ] | length),
        commit_level_primary_owner_review_required_count: ([ $items[] | select(.bucket_id == $id and .commit_level_primary_owner_review_required == true) ] | length)
      })
  '
)"
owner_plan_release_commit_count="$(jq '[.owner_rows[] | select(.primary_owner == "release_truth_and_public_boundary") | .commit_count] | add' "$OWNER_PLAN_JSON")"
owner_plan_release_bucket_count="$(jq '[.owner_rows[] | select(.primary_owner == "release_truth_and_public_boundary")] | length' "$OWNER_PLAN_JSON")"
owner_plan_release_commit_level_required_count="$(jq '[.owner_rows[] | select(.primary_owner == "release_truth_and_public_boundary" and .commit_level_primary_owner_review_required == true) | .commit_count] | add' "$OWNER_PLAN_JSON")"
release_queue_item_count="$(jq 'length' <<<"$queue_items_json")"
release_queue_commit_level_required_count="$(jq '[.[] | select(.commit_level_primary_owner_review_required == true)] | length' <<<"$queue_items_json")"
truth_source_review_item_count="$((release_queue_item_count - release_queue_commit_level_required_count))"
origin_commit="$(git -C "$ROOT" rev-parse origin/main)"
head_commit="$(git -C "$ROOT" rev-parse HEAD)"
dirty_count="$(git -C "$ROOT" status --porcelain | wc -l | tr -d ' ')"

jq -n \
  --arg contract_version "trillionnium_world_review_release_owner_queue_v1" \
  --arg status "review_release_owner_queue_ready" \
  --arg generated_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  --arg doc_path "$DOC_REL" \
  --arg origin_commit "$origin_commit" \
  --arg head_commit "$head_commit" \
  --argjson dirty_count "$dirty_count" \
  --argjson owner_plan_release_bucket_count "$owner_plan_release_bucket_count" \
  --argjson owner_plan_release_commit_count "$owner_plan_release_commit_count" \
  --argjson owner_plan_release_commit_level_required_count "$owner_plan_release_commit_level_required_count" \
  --argjson release_queue_item_count "$release_queue_item_count" \
  --argjson release_queue_commit_level_required_count "$release_queue_commit_level_required_count" \
  --argjson truth_source_review_item_count "$truth_source_review_item_count" \
  --argjson bucket_counts "$bucket_counts_json" \
  --argjson queue_items "$queue_items_json" \
  '{
    ...
    primary_owner: "release_truth_and_public_boundary",
    lane_bucket_count: $owner_plan_release_bucket_count,
    lane_bucket_ids: [
      "multi_public_boundary_overlap",
      "multi_release_native_handoff_overlap",
      "unclassified_generated_count_surface",
    ...
    owner_plan_release_commit_count: $owner_plan_release_commit_count,
    release_queue_item_count: $release_queue_item_count,
    queue_matches_owner_plan: ($release_queue_item_count == $owner_plan_release_commit_count),
    owner_plan_commit_level_required_count: $owner_plan_release_commit_level_required_count,
    commit_level_primary_owner_review_required_count: $release_queue_commit_level_required_count,
    truth_source_review_item_count: $truth_source_review_item_count,
    bucket_counts: $bucket_counts,
    queue_items: $queue_items,
    bucket_coverage_complete: (
      ($bucket_counts | length) == $owner_plan_release_bucket_count
      and ($release_queue_item_count == $owner_plan_release_commit_count)
      and ($release_queue_commit_level_required_count == $owner_plan_release_commit_level_required_count)
    ),
    ...
    reviewer_next_action: "review queue_items in owner_review_order, resolving public/no-credit and count/doc truth-source issues before any external push or history operation"
  }' >"$SUMMARY"
...
jq -e '
  ...
  and .truth_source_review_item_count >= 1
  ...
  and (.queue_items | length) == .release_queue_item_count
  and (.queue_items | all(.primary_owner == "release_truth_and_public_boundary"))
  and .push_performed == false
  and .rebase_performed == false
  and .reset_performed == false
  and .squash_performed == false
  ...
  and (.no_credit_boundary | contains("local release/public-boundary owner queue only"))
  and (.reviewer_next_action | contains("before any external push or history operation"))
' "$SUMMARY" >/dev/null

{
  printf '# Trillionnium World Review Release-Owner Queue\n\n'
  printf -- '- status: `%s`\n' "$(jq -r '.status' "$SUMMARY")"
  printf -- '- primary owner: `%s`\n' "$(jq -r '.primary_owner' "$SUMMARY")"
  printf -- '- lane buckets: `%s`\n' "$(jq -r '.lane_bucket_count' "$SUMMARY")"
  printf -- '- queue items: `%s`\n' "$(jq -r '.release_queue_item_count' "$SUMMARY")"
  printf -- '- commit-level primary-owner review required: `%s`\n' "$(jq -r '.commit_level_primary_owner_review_required_count' "$SUMMARY")"
  printf -- '- truth-source/count review items: `%s`\n' "$(jq -r '.truth_source_review_item_count' "$SUMMARY")"
  ...
  jq -r '.bucket_counts[] | "- `\(.bucket_id)`: `\(.queue_item_count)` commits; commit-level owner review `\(.commit_level_primary_owner_review_required_count)`"' "$SUMMARY"
  printf '\n## Queue Sample\n\n'
  jq -r '.queue_items[:20][] | "- `\(.short)`: \(.subject) (`\(.bucket_id)`)"' "$SUMMARY"
} >"$SUMMARY_MD"

printf 'TRILLIONNIUM_WORLD_REVIEW_RELEASE_OWNER_QUEUE_GREEN %s %s\n' "$SUMMARY" "$SUMMARY_MD"
