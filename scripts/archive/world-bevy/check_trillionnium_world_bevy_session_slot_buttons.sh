#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
EVIDENCE_DIR="$ROOT/acceptance/S5_native_bevy_device/latest"
mkdir -p "$EVIDENCE_DIR"

SUMMARY="$EVIDENCE_DIR/bevy-session-slot-buttons.json"
SLOT="$EVIDENCE_DIR/bevy-session-slot-buttons.snapshot.json"

(
  cd "$ROOT/trillionnium"
  TRNM_WORLD_BEVY_SESSION_SAVE_SLOT_PATH="$SLOT" \
    cargo run -p trnm-world-bevy -- session-slot-buttons "$SLOT" >"$SUMMARY"
)

jq -e '
  .contract_version == "trillionnium_world_bevy_session_slot_buttons_v1"
  and .session_save_slot_contract == "trillionnium_world_bevy_session_save_slot_v1"
  and .session_recovery_ui_contract == "trillionnium_world_bevy_session_recovery_ui_v1"
  and .state_snapshot_contract == "trillionnium_world_bevy_state_snapshot_v1"
  and .green == true
  and .save_button_gate == true
  and .load_button_gate == true
  and .slot_file_gate == true
  and .restored_ui_gate == true
  and .continue_after_load_gate == true
  and .post_restore_guard_gate == true
  and .post_restore_continue_gate == true
  and .final_hud_gate == true
  and .slot_exists == true
  and .slot_bytes > 512
  and (.expected_slot_path | endswith("bevy-session-slot-buttons.snapshot.json"))
  and .expected_slot_path == .action_slot_path
  and .save_button_event.action == "SAVE:SLOT"
  and .save_button_event.accepted == true
  and .save_button_event.availability_before == "enabled_session_checkpoint"
  and .load_button_event.action == "LOAD:SLOT"
  and .load_button_event.accepted == true
  and .load_button_event.availability_before == "enabled_session_slot_found"
  and (.after_load_sample.session_resume_text | contains("RESUME ACTIVE"))
  and .after_load_sample.runtime.session_resume_input_locked == true
  and .continue_after_load_event.action == "CONTINUE:SESSION"
  and .continue_after_load_event.accepted == true
  and .continue_after_load_event.availability_before == "enabled_session_resume_continue"
  and .after_continue_sample.runtime.session_resume_input_locked == false
  and .post_restore_blocked_event.action == "EQUIP:bandit_sash"
  and .post_restore_blocked_event.accepted == false
  and .post_restore_blocked_event.availability_before == "item_not_in_bag:bandit_sash"
  and .post_restore_blocked_event.core_state_unchanged_when_disabled == true
  and .post_restore_equip_event.action == "EQUIP"
  and .post_restore_equip_event.accepted == true
  and .final_input_telemetry_summary.total_events == 12
  and .final_input_telemetry_summary.accepted_events == 8
  and .final_input_telemetry_summary.blocked_events == 4
  and .final_input_telemetry_summary.keyboard_events == 10
  and .final_input_telemetry_summary.bevy_button_events == 2
  and .final_input_telemetry_summary.last_action_label == "EQUIP"
  and (.final_session_text | contains("SESSION RECOVERED"))
  and (.final_session_text | contains("checkpoint events 12 accepted 8 blocked 4"))
  and (.final_session_text | contains("last keyboard EQUIP"))
  and (.final_session_text | contains("guard EQUIP:bandit_sash:item_not_in_bag:bandit_sash"))
  and (.final_event_log_text | contains("INPUT SUMMARY total 12 accepted 8 blocked 4 keyboard 10 buttons 2"))
  and .final_runtime.objective_status == "first_playable_loop_complete"
  and .final_runtime.equipment_ready == true
  and .android_s5_real_device_claimed == false
' "$SUMMARY" >/dev/null

jq -e '
  .contract_version == "trillionnium_world_bevy_state_snapshot_v1"
  and .actor_id == "local-player"
  and (.first_playable.input_feedback_history | length == 10)
' "$SLOT" >/dev/null

printf 'TRILLIONNIUM_WORLD_BEVY_SESSION_SLOT_BUTTONS_GREEN %s slot=%s\n' "$SUMMARY" "$SLOT"
