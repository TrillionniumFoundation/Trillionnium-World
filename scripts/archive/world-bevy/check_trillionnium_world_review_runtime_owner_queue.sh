...
DOC_REL="docs/archive/world-review-2026-07/trillionnium-world-review-runtime-owner-queue-2026-07-07.md"
DOC="$ROOT/$DOC_REL"
OWNER_PLAN_JSON="$ACCEPTANCE_DIR/trillionnium-world-review-primary-owner-plan.json"
SUMMARY="$ACCEPTANCE_DIR/trillionnium-world-review-runtime-owner-queue.json"
SUMMARY_MD="$ACCEPTANCE_DIR/trillionnium-world-review-runtime-owner-queue.md"
...
bucket_queue_role() {
  case "$1" in
    multi_native_bevy_rts_boundary_overlap)
      printf 'Review renderer-neutral RTS contracts before Bevy draw/runtime integration.'
      ;;
    unclassified_bot_executor_surface)
      printf 'Decide whether bot/executor changes belong to RTS runtime/data, Bevy integration, or release evidence.'
      ;;
    unclassified_map_or_modeling_surface)
      printf 'Verify no live ingestion or public map-pack credit is implied, then assign owner slice.'
      ;;
    *)
      printf 'Not part of the RTS runtime/data-boundary owner queue.'
      ;;
  esac
}
...
bucket_review_order() {
  case "$1" in
    multi_native_bevy_rts_boundary_overlap) printf '3' ;;
    unclassified_bot_executor_surface) printf '6' ;;
    unclassified_map_or_modeling_surface) printf '8' ;;
    *) printf '99' ;;
  esac
}
...
bucket_requires_commit_level_owner() {
  case "$1" in
    multi_native_bevy_rts_boundary_overlap) printf 'true' ;;
    *) printf 'false' ;;
  esac
}

is_runtime_owner_bucket() {
  case "$1" in
    multi_native_bevy_rts_boundary_overlap|unclassified_bot_executor_surface|unclassified_map_or_modeling_surface)
      return 0
      ;;
    *)
      return 1
...
if [[ ! -f "$DOC" ]]; then
  echo "[FAIL] missing review runtime-owner queue doc: $DOC" >&2
  exit 1
fi

require_text "$DOC" "Status: local RTS runtime/data-boundary owner queue."
...
require_text "$DOC" '| `unclassified_map_or_modeling_surface` |'
...
queue_jsonl="$TMP_DIR/runtime_owner_queue.jsonl"
: >"$queue_jsonl"
queue_order=0
...
while IFS=$'\t' read -r hash subject; do
  ...
  if ! is_runtime_owner_bucket "$bucket"; then
    continue
  fi
...
queue_items_file="$TMP_DIR/runtime_owner_queue.json"
bucket_counts_file="$TMP_DIR/runtime_owner_bucket_counts.json"
...
jq -n --slurpfile queue_items "$queue_items_file" '
  ...
  |
    ...
    | map(. as $id | {
        ...
        owner_review_order: (
          if $id == "multi_native_bevy_rts_boundary_overlap" then 3
          elif $id == "unclassified_bot_executor_surface" then 6
          else 8
          end
        ),
...
' >"$bucket_counts_file"
owner_plan_runtime_commit_count="$(jq '[.owner_rows[] | select(.primary_owner == "rts_runtime_data_boundaries") | .commit_count] | add' "$OWNER_PLAN_JSON")"
owner_plan_runtime_bucket_count="$(jq '[.owner_rows[] | select(.primary_owner == "rts_runtime_data_boundaries")] | length' "$OWNER_PLAN_JSON")"
owner_plan_runtime_commit_level_required_count="$(jq '[.owner_rows[] | select(.primary_owner == "rts_runtime_data_boundaries" and .commit_level_primary_owner_review_required == true) | .commit_count] | add' "$OWNER_PLAN_JSON")"
runtime_queue_item_count="$(jq 'length' "$queue_items_file")"
runtime_queue_commit_level_required_count="$(jq '[.[] | select(.commit_level_primary_owner_review_required == true)] | length' "$queue_items_file")"
runtime_boundary_review_item_count="$((runtime_queue_item_count - runtime_queue_commit_level_required_count))"
...
jq -n \
  --arg contract_version "trillionnium_world_review_runtime_owner_queue_v1" \
  --arg status "review_runtime_owner_queue_ready" \
  --arg generated_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  --arg doc_path "$DOC_REL" \
  --arg origin_commit "$origin_commit" \
  --arg head_commit "$head_commit" \
  --argjson dirty_count "$dirty_count" \
  --argjson owner_plan_runtime_bucket_count "$owner_plan_runtime_bucket_count" \
  --argjson owner_plan_runtime_commit_count "$owner_plan_runtime_commit_count" \
  --argjson owner_plan_runtime_commit_level_required_count "$owner_plan_runtime_commit_level_required_count" \
  --argjson runtime_queue_item_count "$runtime_queue_item_count" \
  --argjson runtime_queue_commit_level_required_count "$runtime_queue_commit_level_required_count" \
  --argjson runtime_boundary_review_item_count "$runtime_boundary_review_item_count" \
  ...
  '{
    ...
    primary_owner: "rts_runtime_data_boundaries",
    lane_bucket_count: $owner_plan_runtime_bucket_count,
    lane_bucket_ids: [
      "multi_native_bevy_rts_boundary_overlap",
      "unclassified_bot_executor_surface",
      "unclassified_map_or_modeling_surface"
    ],
    owner_plan_runtime_commit_count: $owner_plan_runtime_commit_count,
    runtime_queue_item_count: $runtime_queue_item_count,
    queue_matches_owner_plan: ($runtime_queue_item_count == $owner_plan_runtime_commit_count),
    owner_plan_commit_level_required_count: $owner_plan_runtime_commit_level_required_count,
    commit_level_primary_owner_review_required_count: $runtime_queue_commit_level_required_count,
    runtime_boundary_review_item_count: $runtime_boundary_review_item_count,
    ...
    bucket_coverage_complete: (
      ($bucket_counts_file[0] | length) == $owner_plan_runtime_bucket_count
      and ($runtime_queue_item_count == $owner_plan_runtime_commit_count)
      and ($runtime_queue_commit_level_required_count == $owner_plan_runtime_commit_level_required_count)
    ),
    ...
    reviewer_next_action: "review queue_items in owner_review_order, confirming renderer-neutral RTS ownership before Bevy integration review and before any external push or history operation"
  }' >"$SUMMARY"
...
jq -e '
  ...
  and (.bucket_counts | length) == 3
  and ([.bucket_counts[].bucket_id] | sort) == ([
      "multi_native_bevy_rts_boundary_overlap",
      "unclassified_bot_executor_surface",
      "unclassified_map_or_modeling_surface"
    ] | sort)
  and (.queue_items | length) == .runtime_queue_item_count
  and (.queue_items | all(.primary_owner == "rts_runtime_data_boundaries"))
  and .push_performed == false
  and .rebase_performed == false
  and .reset_performed == false
  and .squash_performed == false
  ...
  and (.no_credit_boundary | contains("local RTS runtime/data-boundary owner queue only"))
  and (.reviewer_next_action | contains("before any external push or history operation"))
' "$SUMMARY" >/dev/null

{
  printf '# Trillionnium World Review Runtime-Owner Queue\n\n'
  printf -- '- status: `%s`\n' "$(jq -r '.status' "$SUMMARY")"
  ...
  printf -- '- runtime/bot/map boundary review items: `%s`\n' "$(jq -r '.runtime_boundary_review_item_count' "$SUMMARY")"
...
printf 'TRILLIONNIUM_WORLD_REVIEW_RUNTIME_OWNER_QUEUE_GREEN %s %s\n' "$SUMMARY" "$SUMMARY_MD"
