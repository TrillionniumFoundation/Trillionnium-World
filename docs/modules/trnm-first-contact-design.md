...
Primary source is `trillionnium/crates/trnm-first-contact/src/`; assets are under `assets/first_contact/`, and packaging inputs are under `packaging/`. Normative documents include `docs/development/trillionnium-rpg-rts-closed-loop-v1.md`, `docs/development/trnm-native-game-release-gates-v1.md`, `docs/modules/trnm-campaign-core-design.md`, `docs/modules/trnm-rts-sim-design.md`, `docs/modules/trnm-online-protocol-design.md`, and `docs/operations/trnm-game-server-runtime-configuration.md`. Executable evidence is produced by `scripts/check_trnm_game_product.sh`, `scripts/check_trillionnium_world_release_review_ci_gate.sh`, `scripts/check_trnm_game_package.sh`, and the crate tests under `trillionnium/crates/trnm-first-contact/src/`. Tests cover plugin/system order, input profiles, maps/assets, closed-loop campaign, local/attached authority separation, snapshots/deltas/reconnect, SIGKILL command-journal recovery, replay, corrupt files, frame stalls, packaging/install, and supported platforms. Automated screenshots cannot satisfy independent human evidence.
