...
Primary source is `trillionnium/crates/trnm-game-server/src/`; migrations are under `trillionnium/crates/trnm-game-server/migrations/`, and integration targets are under `trillionnium/crates/trnm-game-server/tests/`. Normative documents include `docs/database/trnm-world-postgres-contract-v1.md`, `docs/database/trnm-world-lock-order-v1.md`, `docs/architecture/trnm-settlement-lifecycle-v2.md`, `docs/security/trnm-world-service-identity-secrets-v1.md`, `docs/protocol/trnm-world-http-api-v1.md`, `docs/protocol/trnm-world-websocket-v1.md`, and `docs/development/trnm-world-testing-strategy-v2.md`. Executable evidence is produced by `scripts/check_trnm_game_product.sh`, `scripts/check_trnm_settlement_transaction_boundary.sh`, `scripts/run-trnm-world-authority-postgres-check.sh`, and the all-target Rust tests and strict Clippy gates. Required evidence covers PostgreSQL black-box schema/procedures/privileges, two-worker/stale lease, response loss/kill/cancel, journal corruption, shutdown/drain, PITR/timeline/old primary, package/supply chain, exact-head and prospective merge.
