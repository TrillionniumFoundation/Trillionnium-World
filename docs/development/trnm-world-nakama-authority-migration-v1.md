...
- a missing or unknown profile fails closed;
- retries retain the original match ID and authority profile;
- deployment rollback may route **new** matches to an earlier approved Nakama version, never to a simultaneous World authority.
...
The following fields are forbidden at the World contract boundary:
...
### M3 — shadow verification

Two shadow modes are allowed, but never in one match:

1. For an existing legacy match, World remains the sole external authority; Nakama may consume a mirrored stream only as an unsigned, non-settling verifier.
2. For a new Nakama test match, Nakama is the sole external authority; an isolated World legacy process may execute only an unsigned verifier role.

A shadow verifier cannot:

- issue an externally valid receipt;
- sign completion evidence;
- mutate CEX;
- submit Chain ingress;
- influence routing after mismatch;
- become authority during recovery.
...
Exit: sustained vector and live comparison passes with mismatch triage and resource bounds.
...
- legacy clients and matches remain explicitly routed to the legacy profile and receive no target release credit.

Rollback: stop creation of new closed-alpha matches and continue, terminate or quarantine existing Nakama matches through Nakama. Do not send them to World.
...
- Nakama owns canonical event, roster and archive roots;
- Nakama constructs and signs versioned completion evidence;
- World unsigned outcome hash is a bound input, not completion authority;
...
- Chain ingress consumes the published target evidence contract and owns finality and inclusion;
- World does not call target Chain mutation directly;
- idempotency binds match, completion and intent identity across adapters;
- local legacy settlement is separately labeled and cannot be confused with target evidence.

Rollback: disable new value or Chain submission and keep completed matches pending. Do not reissue under a different authority identity.
...
- enumerate all remaining waiting, running, terminal and pending legacy matches;
...
- preserve database, journal and evidence under approved retention and verification tooling;
- remove or permanently compile/runtime-gate legacy authority after retention obligations are met.

Exit: no supported client can create or reconnect a new active legacy match, no active legacy credential remains, and historical verification grants no current authority.

## 6. No-dual-authority protocol

For every match, this tuple is immutable:
...
- a command receipt comes from a different profile or version;
- a completion signature does not bind the tuple;
- CEX or Chain adapter evidence uses a different profile/component lock;
- a client attempts cross-profile reconnect;
- deployment rollback would make a second authority reachable.

A shadow verifier has a separate non-authoritative execution identity and cannot produce an externally valid authority receipt.

## 7. Data and evidence disposition

Legacy data classes include:

- match and campaign rows;
- commands and replay frames;
- hot/cold publication journal evidence;
- terminal ACK and abandonment markers;
- progression and rating events;
- CEX intent/receipt bindings;
- operational and moderation records.

For every class, document:

- system of record and schema version;
- retention duration and legal/product rationale;
- encryption and access principal;
...
| Credential | Legacy owner | Target action |
| --- | --- | --- |
...
| Moderator credential | Operations | Independent from game and signer principals |
| Chain ingress principal | Target adapter | Never placed in World runtime |

No shared root secret may derive multiple target roles. Rotation must be rehearsed before public value credit.

## 9. Failure and rollback matrix

| Event | Required behavior |
| --- | --- |
| Nakama unavailable before match creation | Reject or queue target creation; no World fallback |
...
| Completion signing unavailable | Match may be terminal internally but remains unpublished and unsettled |
| CEX unavailable | Completion remains durable; exact intent retries later |
| Chain unavailable or ambiguous | Preserve exact submission identity; query/retry under Chain contract |
| Deployment rollback | Stop new affected matches; existing matches stay with original profile |
| Legacy server failure during drain | Same-host legacy recovery or explicit fail-close/quarantine; never Nakama takeover |
| Credential compromise | Revoke affected profile/version, stop new matches and quarantine unverifiable evidence |
...
- proof that shadow verifier cannot sign, settle, publish or influence routing;
- Nakama crash at every durable boundary;
- archive-root reconstruction and tamper detection;
- completion-signature key rotation and revocation;
...
Metrics and logs are partitioned by authority profile and contract version:

- match creation, admission and rejection;
- active, waiting, terminal and quarantined counts;
- command duplicate, order and recovery outcomes;
- runtime invocation latency and deterministic mismatch;
- completion publication and signing state;
...
1. World runtime types, hash, schema and vectors.
...
5. Nakama admission and roster.
6. Nakama command ordering and idempotency.
7. Nakama restart recovery and archive.
