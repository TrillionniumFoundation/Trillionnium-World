...
2026-05-19 的 low-spec classic renderer 继续按 RTS 标准补齐本地证据：`scripts/check_trillionnium_world_bevy_classic_rts_control_loop.sh` 生成 `acceptance/S5_native_bevy_device/latest/bevy-classic-rts-control-loop.json` 与 `.ppm`，要求控制组 1 同时覆盖多单位选择、移动目的地、编队线、攻击目标、命令 marker 和攻击反馈像素。该 gate 已纳入 `scripts/check_trillionnium_world_bevy_classic_playtest_readiness.sh` 与 release-review CI，用来防止只堆静态美术而没有 RTS 可操作反馈；它仍是 host-side Bevy classic 证据，不声明 Android 真机或公网发布 ready。
