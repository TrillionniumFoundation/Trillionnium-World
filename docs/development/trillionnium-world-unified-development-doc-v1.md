...
Classic playtest runner status 也进入 packet-level semantic chain：`scripts/check_trillionnium_world_bevy_classic_playtest_runner_status.sh` 由 `classic_playtest_runner_status_semantics` 直接校验 live `trillionnium-bevy-playtest.service`、release `trnm-world-bevy run` 二进制、low-spec classic renderer env、manifest/override paths、工作目录和 CEX path rejection。
