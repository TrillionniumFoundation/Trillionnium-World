...
| --- | --- | --- |
| `reviewer_summary` | Small JSON/Markdown status, packet, runbook, curation, review-slice strategy/manifest, review triage queue, primary-owner plan, release-owner queue, runtime-owner queue, residual queue, review execution batches, public-boundary batch, blocker-ledger, and blocker artifacts. | Read first for state, next action, and no-credit boundaries. |
