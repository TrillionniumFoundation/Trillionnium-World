...
- Visible First Contact player-screen polish has landed for title-bar wording, resource readout spacing, camera-tile formatting, selected-unit display count, production/order/build/completion labels, FIELD STATUS and right-rail TACTICS wording, map/radar readability context, command-grid icon differentiation, full-game aggregate command-grid readability, bottom selection/status panel readability, and terrain/unit/building silhouette readability. The First Contact label, visual, radar, command-grid, full-game visual/UI, bottom-panel, and silhouette guards plus packet integrity checks now reject raw debug/internal display fragments, generic/samey command-grid regressions, raw completion/status text in those lanes, and map-object regressions that collapse worker/scout/warden/relay units or command-core/relay/beacon structures back into samey blocks. The artifact command runner also leaves `TRNM_WORLD_BEVY_CLASSIC_PLAYER_SCREEN=0` by default unless a caller explicitly opts in, so source-artifact refreshes are not polluted by player-screen mode.

### Next Local Slices

1. Keep burning down visible playable-screen polish before adding more abstract proof. Good next targets are stronger live screenshot probes that catch empty/dead panels, clipped labels, debug title text, and non-gameplay-looking screenshots, then deeper terrain/building art passes that replace more placeholder rectangles with authored shapes. Add guards only when they catch real player-facing regressions.
