...
1. Move remaining renderer-neutral First Contact command-surface, runtime UI, and evidence helper code out of `trnm-world-bevy/src/lib.rs` into `trnm-rts-bevy-runtime`, `trnm-rts-online`, or `trnm-rts-evidence`, leaving `trnm-world-bevy` as the adapter/rendering owner. The data-owned player-screen runtime application, offline-adapter runtime application, offline-adapter lobby-ready review, offline-adapter session-transition review, online-owned review-input mapping, player-screen consumption review, campaign UI continuity review, session-state continuity review, continuous player-flow review, live-session playthrough review, full-game visual/UI aggregate review, OpenRA-style screen-set review, and packet assembly review have moved; continue with other remaining local session surfaces and renderer-neutral packet/evidence helpers.
2. Continue extending the Bevy-facing offline adapter path beyond the player-screen/runtime application handoff into broader local session/UI transitions and additional review surfaces. The next slice should consume more screen transitions from the `trnm_rts_online_offline_adapter_runtime_handoff_v1` source and review them through Bevy-free runtime/evidence contracts rather than re-deriving fixture semantics inside `trnm-world-bevy`, while keeping it explicitly no-socket, no-hosted-service, no-client-prediction, no-rollback-netcode, and no-public-launch until real network evidence exists. The newly landed lobby-ready, session-transition, campaign continuity, session-state continuity, continuous player-flow, live-session playthrough, full-game visual/UI aggregate, OpenRA-style screen-set, and packet assembly reviews cover local ready state, command-surface/route-overlay replacement, campaign handoff persistence, save/load/resume UI continuity, six-step local player-flow continuity, same-process playthrough, aggregate UI coverage, scoped OpenRA-style screen-set boundaries, and release packet handoff manifest integrity.
