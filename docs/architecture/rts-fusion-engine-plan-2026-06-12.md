...
- The 2026-06-19/20 work has shifted from boundary-only extraction back to the visible player screen because the highest product risk is still "does this look and read like a real RTS?" The live runner title is concise and player-facing, resource readouts render `CREDITS` / `POWER` / `SUPPLY` / `VISION` with spacing, camera tiles render as `5/4` in visible HUD text, active selected groups no longer display as zero units, production/order/build/completion labels use player-facing names, FIELD STATUS and TACTICS lanes render compact readable detail rows, the First Contact map has selected-squad rings, route marks, objective/structure/lane readability cues, the radar now carries visibility/fog/route/objective/structure/pressure/viewport context, the command grid now has six role-specific glyph signatures instead of repeated generic buttons, and the bottom selection/status panel now renders concise squad/status information such as `GROUP 1 SECURING RELAY`, `SIGNAL BLADE READY`, `WATCH TOWER READY`, and `WORKER` / `SCOUT` / `GUARD` / `RELAY` role chips instead of raw RTS completion strings.
- `first_contact_player_screen_label_guard_v1`, `first_contact_visual_readability_v1`, `first_contact_radar_readability_v1`, `first_contact_command_grid_readability_v1`, and `first_contact_bottom_panel_readability_v1` now keep player-facing HUD labels, resource spacing, build-palette fit, order/completion labels, TACTICS/FIELD STATUS wording, map readability overlays, radar context, command-grid role sequence, unique icon signatures, active-slot state, cooldown badge semantics, bottom-panel feedback wording, selected-count density, and squad-role chip fit under First Contact spec, packet integrity, v2 contract, and fixture coverage.
- Release-review CI is green on the local release artifact path with 377 checks, 0 failures, 128 packet artifacts, and a latest verified total of 182.373 seconds after the bottom-panel readability slice. The current dominant local slow checks are live-window screenshot evidence (17.156s), packet semantic fixtures (7.997s), and packet integrity (6.670s). Live-window screenshot evidence now emits capture/readiness diagnostics so future speed work can distinguish window readiness, frame capture retries, and settle/key retries before trimming any proof.
