...
2. Extend the Bevy-facing offline adapter path into actual local UI/action replay consumption while keeping it explicitly no-socket, no-hosted-service, no-client-prediction, no-rollback-netcode, and no-public-launch until real network evidence exists.
