...
2. Continue extending the Bevy-facing offline adapter path beyond the new player-screen/session handoff surface into broader local session/UI transitions and additional review surfaces. The next slice should consume more screen transitions from the `trnm_rts_online_offline_adapter_runtime_handoff_v1` source rather than re-deriving fixture semantics inside `trnm-world-bevy`, while keeping it explicitly no-socket, no-hosted-service, no-client-prediction, no-rollback-netcode, and no-public-launch until real network evidence exists.
