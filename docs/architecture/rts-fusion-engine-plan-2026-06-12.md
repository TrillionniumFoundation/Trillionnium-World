...
- `trnm-rts-evidence` exists and assembles deterministic Bevy runtime adapter evidence before `trnm-world-bevy` includes that proof in release-review evidence. It also owns the campaign UI continuity review contract for campaign handoff final/restored route state, save-restore persistence, milestone pixels, native-client no-credit boundary, and player-first route-resume screen gates. It now also owns the session-state continuity review contract for save-slot confirmation, load-resume lock/continue, recovery guard, match setup, restored HUD, campaign outcome, campaign continuity, source preview readiness, native-client no-credit boundaries, and the player-first session resume screen. It now owns the continuous player-flow review contract for the six-step title/account -> match setup -> in-match HUD -> command feedback -> save/resume -> outcome/open-world local runtime surface, including player-first screen and no-credit boundaries. It also owns the live-session playthrough review contract for the same-process trace sidecar, live RTS command acceptance, slot A save/load/resume, final open-world handoff, player-first tactical screen, and no-credit boundary. The release-review packet now also carries a `trnm_rts_evidence_release_review_packet_assembly_review_v1` object that reviews manifest completeness, missing-artifact state, key RTS runtime artifacts, packet semantic fixtures, status handoff, public-launch no-credit boundary, and external blocker state before reviewer handoff.
...
- Release-review evidence assembly is split into `trnm-rts-evidence`, including the campaign UI continuity handoff review, session-state continuity review, continuous player-flow review, live-session playthrough review, and release-review packet assembly review.
