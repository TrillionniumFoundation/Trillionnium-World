...
- Visible First Contact player-screen polish has landed for title-bar wording, resource readout spacing, camera-tile formatting, selected-unit display count, production/order/build/completion labels, FIELD STATUS and right-rail TACTICS wording, map/radar readability context, command-grid icon differentiation, full-game aggregate command-grid readability, and bottom selection/status panel readability. The First Contact label, visual, radar, command-grid, full-game visual/UI, and bottom-panel guards plus packet integrity checks now reject raw debug/internal display fragments, generic/samey command-grid regressions, and raw completion/status text in those lanes. The artifact command runner also leaves `TRNM_WORLD_BEVY_CLASSIC_PLAYER_SCREEN=0` by default unless a caller explicitly opts in, so source-artifact refreshes are not polluted by player-screen mode.

### Next Local Slices

1. Keep burning down visible playable-screen polish before adding more abstract proof. Good next targets are original terrain/unit/building silhouette readability for First Contact Basin, stronger live screenshot probes that catch empty/dead panels, clipped labels, debug title text, and non-gameplay-looking screenshots, and only then deeper terrain/building art passes. Add guards only when they catch real player-facing regressions.
