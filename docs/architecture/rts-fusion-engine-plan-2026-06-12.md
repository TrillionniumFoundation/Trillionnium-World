...
- The 2026-06-19 work has shifted from boundary-only extraction back to the visible player screen because the highest product risk is still "does this look and read like a real RTS?" The live runner title is concise and player-facing, resource readouts render `CREDITS` / `POWER` / `SUPPLY` / `VISION` with spacing, camera tiles render as `5/4` in visible HUD text, active selected groups no longer display as zero units, production/order/build/completion labels use player-facing names, and the TACTICS queue summary now renders compact readable progress such as `WORKER 42% TOWER 66%` instead of internal `P:` / `B:` / `@` / underscore text.
- `first_contact_player_screen_label_guard_v1` now keeps the player-facing HUD label set, resource spacing, build-palette fit, order/completion labels, TACTICS queue summary width, and raw-marker rejection under First Contact spec, packet integrity, v2 contract, and fixture coverage.
