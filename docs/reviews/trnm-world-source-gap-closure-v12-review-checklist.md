...
- Verify the server reports `server_version_num=160004`.
...
- Confirm `trnm-game-ci`, `trnm-world-p0-boundaries`, and `trnm-world-status-evidence` are emitted by the reviewed workflow object.
- Confirm `trnm-world-v12/required` depends on terminal success of all three lanes.
